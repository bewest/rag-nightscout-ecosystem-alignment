...
**Method**: For each of the 10 excursion types, used a leave-one-out
mean-trajectory prediction: for each trajectory, the prediction is the
mean of all other trajectories of the same type from the same patient.
No engineered features (supply, demand, IOB, etc.) are used — only the
raw glucose shape. This gives a "perfect hindsight" ceiling: the maximum
predictable variance given perfect type classification.
...
6. **Information ceiling**: Uses leave-one-out mean-trajectory prediction
   (no engineered features). The negative R² indicates that even the mean
   shape of same-type trajectories is a worse predictor than the global
   mean. This strongly suggests genuine information deficit — the excursion
   types do not capture enough regularity to predict individual trajectories.
...
The "information ceiling" concept assumes that leave-one-out mean-trajectory
prediction approximates the best achievable type-based predictor; with
richer features (supply-demand, IOB, COB), the actual ceiling may be higher.*
