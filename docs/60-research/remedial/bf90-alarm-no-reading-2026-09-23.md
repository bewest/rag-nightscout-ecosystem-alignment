...
> **Snapshot: measured 2026-09-22 (US local; run clock 2026-09-23 UTC) against `origin/master 92d08342` (tag 15.0.8), `origin/dev 74fc6619`, and local branch `bf3/alarm-no-reading` at `92544d8f` (one commit on `74fc6619`). Nothing here is pushed or released. This is evidence for a maintainer review, not a decision. Current facts about the defect live in the [backfix register](../../30-design/remedial/nightscout-backfix-register.md) (BF-90) and item `BFQ-90` in [`queue/work-queue.yaml`](../../../queue/work-queue.yaml).**
