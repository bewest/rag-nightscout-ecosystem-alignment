...
connector `dev` `d208c7d` it logs fixed debug messages only.
...
**Fix**: move `master` and cuts 1-3 onto the connector release P0-TAG cuts from connector `dev`.
The release must exist first — see the
...
`nightscout-connect` v0.0.13 and connector `dev` `d208c7d` declare `dependencies.axios = "^1.18.1"`.
...
mapping. It is in connector `dev` `d208c7d` (PR #64, 2026-09-22), not in any connector release or
in the `234d47c` pin 15.0.9 ships, so it reaches operators through P0-TAG and P0-PIN. Nightscout is not a medical device; an operator relying on alarms
