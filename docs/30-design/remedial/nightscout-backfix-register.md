...
`origin/master` and its tags are what operators install. On 2026-09-24 `origin/master` is **350
commits behind `origin/dev` (`153e5658`) and 0 ahead**
(`git -C externals/cgm-remote-monitor-official rev-list --count official/master..official/dev`),
and the release PR #8598 (`dev` → `master`) is open at `153e5658`, with no review.
...
| **BF-17** | Editing a subject through the stock admin UI **persists the API access token in plaintext**, into a field the server otherwise only derives | `lib/authorization/endpoints.js:38-42` + `lib/admin_plugins/subjects.js:43` + `lib/authorization/storage.js` `save` | **high** — turns read access to the database into API access; no key required | yes | **fixed 2026-09-15** — `bf/auth` `64db1f35`, carried by `bf2/auth-hardening`, in review as PR #8754 (head `e549e1a6`, 2026-09-24; security review by the maintainer and Andy); reproduced live; **existing rows still hold tokens, see the report** |
