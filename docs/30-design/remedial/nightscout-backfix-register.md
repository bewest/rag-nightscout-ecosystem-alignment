...
cgm-remote-monitor `origin/dev` `e3adc91d` and `origin/master` `92d08342` (= tag `15.0.8`, the
...
| `fixed <date>` | repaired on a backfix branch with tests, **not merged**; the row names the branch and commit. A part the maintainer decided to keep as it is, recorded with the reason in the detail section, does not leave the entry `partly fixed` |
| `partly fixed <date>` | part of the repair is on a backfix branch with tests, **not merged**; the row names the branch and commit, and the detail section says what is left and why |
...
| question | answer, 2026-09-26 (`origin/dev` `e3adc91d`) | reproduce with |
|---|---|---|
| How many §1 defects are there, and how many reach an operator on 15.0.8? | **99** §1 defects (every §1 entry except BF-12, invalid, and BF-41, closed): **26 open, 59 merged, 1 partly merged (BF-07), 13 fixed on unmerged local branches** (BF-52, BF-80, BF-106, BF-121, BF-122, BF-123, BF-125, BF-128, BF-129, BF-135, BF-136, BF-138 and BF-139). **97** reach an operator on 15.0.8 — BF-80 and BF-106 exist only on `dev` (below) | `node tools/queue/gates/register-exposure-legend.js` (it counts BF-07 under open) |
...
On 2026-09-26 the highest is BF-139 (BF-136 to BF-139 were filed that day from the maintainer's decisions), so the next free id is BF-140. BF-82, BF-83 and BF-84 are reserved for three connection-pooler
...
| **BF-80** | **Scoping alarm delivery to a room couples it to the failed-login delay list, and a household shares one public IP.** `authorization.resolve()` consults `shouldDelayRequest(ip)` and `await`s the accumulated penalty before returning any permissions. Once `/alarm` delivery depends on a resolved entitlement — which it must, to close BF-75 — a socket from an address that recently failed an authentication is **outside the delivery room for the length of that penalty**, and any alarm emitted in the window does not reach it. The penalty is **cumulative, 5 s per failure** (`authFailDelay`), so a misconfigured uploader retrying with a wrong API secret keeps extending it for every other device behind the same NAT address | `lib/authorization/delaylist.js` (`DELAY_ON_FAIL`, cumulative; `FAIL_AGE` 60 s) reached from `lib/authorization/index.js:155`, via `applyReadEntitlement` on `bf/alarm-socket-scope` | **medium** — an alarm a caregiver is entitled to is not delivered, silently, for tens of seconds, and the trigger is a *neighbouring device's* misconfiguration rather than anything the viewer did. Not a new exposure and not exploitable; it is the cost of the BF-75 fix | yes (once BF-75 lands) | **fixed 2026-09-26** on local branch `bf/alarm-anonymous-no-delay` `8e295769` (one commit on `dev` `e3adc91d`, not pushed; option 1 in this register's numbering, which the decision round called option 2): a `/alarm` subscribe carrying no credential (secret, `jwtToken`, `token` and `accessToken` all absent, null or empty) is answered with `AUTH_DEFAULT_ROLES` without the failed-login delay (new `authorization.resolveAnonymous`; `resolve()` and HTTP unchanged; #8754's anonymous-waits test still passes). On a booted server with the default delay after 3 failed logins: anonymous on a readable site went from ack 9999 ms and no alarm within 20 s to ack 1 ms and the alarm at 13 ms; anonymous on a denied site is answered in 1 ms with `read: false` and still gets no alarm (BF-75 holds). Every credentialed subscribe, valid or not, is unchanged: ack about 10 s, alarm at the next repeat (37.7 s readable, 49.5 s denied). Suite 3180/0/3, 10 new tests (4 fail on `e3adc91d`). Left (detail): a signed-in viewer on a throttled address, a socket that connects without subscribing, and main-namespace chart data still wait. Found 2026-09-21 while reviewing the BF-75 fix; **real on `dev` `e3adc91d`**. **Reproduced** 2026-09-25 (mongod 7.0.43, Node 22.23.2, 75 runs): a device with valid web credentials (JWT or API secret), or an anonymous viewer on a readable site, that connects or reconnects to `/alarm` while its resolved address is in the failed-login delay is admitted only after that delay (≈5·(N−1) s after N overlapping failures, ≈5 s for serial retries), so it misses the alarm as first emitted and receives it at the next re-emission (heartbeat or next reading): **45-56 s late for N = 1-10**, **81.7 s** under `AUTH_DEFAULT_ROLES=denied` with N = 10. A successful authentication does not shorten the wait, because the wait precedes the credential check (`lib/authorization/index.js:152-158`, deliberate). Not affected: different resolved addresses, sockets already in the room, `subscribe({accessToken})` (the path AndroidAPS uses, read). Only a bad `Authorization: Bearer`, a bad `api-secret` header or bad socket credentials count as failures; a bad `?token=` query token does not. Each failure raises an admin-only "Failed authentication" notification naming the address; nothing says alarms are held back. `v15.0.8`: every scenario delivered in 0-2 ms. The earlier 4.9 / 10.0 / 15.0 s figures were admission times, not alarm receipt. **Decision 2026-09-26 (maintainer):** a viewer that presents no credential is not held by the failed-login delay; a viewer that presents one keeps the delay, as now (detail, option 1) |
...
| **BF-106** | A numeric `date` (or `sgv`) filter on API v1 `/activity` **matches nothing and answers 200**: the schema-driven coercion that fixed BF-03 names a `collection` on each storage and, when one is named, drops `query.js`'s default walker (`date`, `sgv` → `parseInt`). `activity`'s schema entry is empty, so the bound stays a string and never matches the stored number. `devicestatus` loses the same default for `sgv` only (its schema still types `date` and `mills`) | `lib/server/query.js` `default_options` (`opts.walker = opts.collection ? { } : …`) with `lib/server/activity.js` `queryOpts.collection = 'activity'`, on `origin/dev` `ddd9b600` and the 15.0.9 candidate; not on `v15.0.8` | **medium** — wrong answer with HTTP 200 for a read filter, and a bulk `DELETE /api/v1/devicestatus` filtered on `sgv` deletes nothing where 15.0.8 deleted the matching records (corrected 2026-09-25: this cell first said no write or delete was affected). No client in the [consumer survey](../../60-research/remedial/consumer-impact-15.0.9-2026-09-23.md) filters activity by `date` | yes (once 15.0.9 ships) | **fixed 2026-09-25** on local branch `bf/activity-date-coercion` `20c197bb` (one commit on `origin/dev` `e3adc91d`, not pushed): when a collection is named and no walker is given, `query.js` defaults to `legacyNumericDefaults(collection)`, reading `date` and `sgv` as numbers (through `coercion.toNumber`) only where the schema does not type them, and `profile.js` sets `walker: {}` again, as on 15.0.8. This restores the activity `date`/`sgv` filters and the devicestatus `sgv` filter, including the devicestatus bulk DELETE. The gate fails on `e3adc91d` and passes on `20c197bb`; 18 new tests; suite 3188/0/3 (Node 22.23.2, MongoDB 7.0.43). Found 2026-09-23 by the consumer survey. **Reproduced**: the same `GET /api/v1/activity.json?find[date][$gte]=<ms>` returns 7 records on `v15.0.8`, 0 on `dev` `ddd9b600` and 0 on the candidate, with the `find[created_at][$gte]` control at 7 on all three (mongod 7.0.43, Node 22.23.2); gate `tools/queue/gates/bf106-activity-date-coercion.js` |
...
| **BF-128** | **On a site whose display units are mmol/L, `/pebble?units=mgdl` returns the reading in mg/dL but the delta (`bgdelta`) in mmol/L**, unlabelled: a reading of 90 mg/dL falling 2 mg/dL comes back as `sgv "90"`, `bgdelta -0.1`. The middleware turns `?units` into `req.mmol`, which picks the reading's units, but `prepareSandbox` only ever forces the sandbox **to** mmol; it never forces mg/dL, so the delta keeps the site's units. The opposite request (`?units=mmol` on an mg/dL site) is consistent | `lib/server/pebble.js` `prepareSandbox` `:143-154` (only-mmol override `:146-148`), middleware `:177`; delta scaled by site units in `lib/plugins/bgnow.js:171`; identical on `v15.0.8` and `dev` `4f705217` | **correctness** — a watch face showing a delta 18 times too small, in the wrong units, next to a correct reading; bounded to mmol/L sites whose client asks `/pebble` for mg/dL | yes | **fixed 2026-09-25** on local branch `bf/pebble-delta-units` `aa224c69` (one commit on `origin/dev` `e3adc91d`, not pushed): `addDelta` uses `delta.mgdl` unless mmol is requested; the sandbox units are unchanged, because switching them (the fix shape first written here) gives a wrong `bwp` (measured below). Probe exit 1 → 0; 21 new tests; suite 3191/0/3 (Node 22.23.2, MongoDB 7.0.43). Filed 2026-09-25 from the GitHub triage (issue #6220). **Reproduced** ([probe](../../../tools/lab/triage-2026-09/pebble-units.js)), in-process through the pebble middleware and handler: `bgdelta -0.1` with `sgv "90"` on `v15.0.8` and `dev` `4f705217`; three controls consistent. The same branch carries BF-139 (`df149642`) and BF-138 (`b06eb014`) on top of `aa224c69`, in one PR (maintainer decision 2026-09-26); one BF-128 test expectation changed by design, because `bwpo` now moves with `?units=mgdl`; suite on `b06eb014` 3216/0/3 |
| **BF-129** | `GET /api/v1/entries/<id>.json` for a 24-hex id that **names no entry answers HTTP 500** with "No such id", where a read that finds nothing answers 200 and `[]`. The route sets `res.entries_err` when `getEntry` returns nothing, and the formatter answers any `entries_err` with 500. On #8758 the route also accepts an upper-case id, so a request that was answered as a `spec` on 15.0.8 (200, `[]`) now reaches the 500 | `lib/api/entries/index.js` `GET /entries/:spec` (`res.entries_err = 'No such id…'`); `v15.0.8` `92d08342` and #8758 `ab7b22d6` (`isId` accepts either case since `a612a26f`) | **low** — a lookup of a missing or deleted reading reads as a server fault; a client that retries on 500, or treats it as an outage, is misled. The consumer survey finds no corpus client that fetches an entry by id | yes | **fixed 2026-09-25** on local branch `bf/entries-unknown-id` `f79dc732` (one commit on `origin/dev` `e3adc91d`, not pushed): an unknown 24-hex id answers 200 `[]`, as every other v1 read that matches nothing (200 `[]` chosen over 404); a storage fault still answers 500; the swagger description says so. 11 new tests; suite 3181/0/3. **Reproduced** 2026-09-25 (Node 22.23.2, MongoDB 7.0.43): an unknown lower-case id answers 500 and an upper-case one 200 `[]` on `v15.0.8`, both 500 on `dev` `e3adc91d`, both 200 `[]` on the branch. Found 2026-09-25 by the #8758 freeze review (queue BFQ-129) |
| **BF-134** | Every Loop remote command (override, override cancel, remote carbs, remote bolus) **leaves a connection to Apple's push service open for the life of the process**. `lib/server/loop.js` builds a new `@parse/node-apn` Provider per command and never calls `shutdown()`; each provider's client keeps its HTTP/2 session open with a 60 s ping, and its heartbeat interval (holding the client and its signed token) stays even after Apple closes the connection. Commands refused after the settings checks also built a provider | `lib/server/loop.js` `sendNotification`, with `@parse/node-apn` 5.2.3 `lib/client.js`; `v15.0.8` `92d08342` and `dev` `4d9ecc3b` | **availability / resource** — one connection and one timer per remote command, growing until restart; bounded to sites with Loop remote commands configured (a key and a device token); Apple's idle closes end the connections but not the timers | yes | **merged 2026-09-25** (PR #8770, merge `e3adc91d`: the provider is built just before the send and shut down when it settles); not released, goes into 15.0.9. **Reproduced**: after 1/5/20 commands, 1/5/20 sessions and heartbeat intervals remain on `v15.0.8` and `dev`, 0 on the branch; after a server GOAWAY the sessions close but all 20 intervals stay live. Controls: one push leaves 1; `shutdown()` leaves 0. Found 2026-09-25 from the #8419 test hang |
| **BF-133** | The carbs-on-board (COB) pill's **"last carbs" detail can name an older carb entry than the newest one**: `cob.js` takes `lastCarbs` as the last carb treatment in the order of the treatments array, not the one with the latest time. When that order changes (for example after an older carb entry is edited), the detail switches to the older entry; the COB total is not affected | `lib/plugins/cob.js` `fromTreatments` (`lastCarbs = treatment` in array order); `v15.0.8` `92d08342`, `dev` `4f705217`, #8758 `ab7b22d6` and `e9dbb1fb` | **low** — a detail line (and `/api/v2/properties` `cob.lastCarbs`) names the wrong carb entry; the COB number shown and used is unchanged. Do not rely on the detail line for the time of the last carbs | yes | **open, found 2026-09-25** by the 15.0.9 A/B soak (queue BFQ-133). **Reproduced**: raw `/api/v2/properties` replies at ticks 45 to 63 of one soak run; after an older 25 g entry is edited, every build switches `lastCarbs` from the newest 15 g entry to it, the ab7b22d6 build one reload earlier than the others. Why the array order changes is not traced |
| **BF-135** | **A record that AndroidAPS deletes keeps counting on the site.** AndroidAPS deletes by marking the record `isValid: false` (an API v3 DELETE, or a websocket `dbUpdate` from its v1 client), and API v3 search and history honour that. Every other reader ignores it: v1 reads, the data loader and so COB, IOB and the chart, the server cache, `/api/v1/profile/current` and `/api/v2/properties` keep serving and counting the record. Outside API v3, only the `OpenAPS Offline` handling checks `isValid` | `lib/data/ddata.js:351` (the one `isValid` check outside API v3); the v1 storage reads in `lib/server/treatments.js`, `entries.js`, `devicestatus.js`, `profile.js` and `food.js`, `lib/server/cache.js`, and the websocket `dbAdd` dedup lookup; `v15.0.8` `92d08342` and `dev` `e3adc91d` | **safety** (dosing-relevant display): carbs or insulin deleted in AndroidAPS, or through any API v3 DELETE, keep counting in the COB and IOB Nightscout shows and hands to followers, so both are overstated with no error. Bounded to records deleted by soft delete. Check COB and IOB in the app that doses | yes | **fixed 2026-09-25** on local branch `bf/v1-writes-v3-history` `718efddc` (the BF-122 commit, on `origin/dev` `e3adc91d`, not pushed; the branch head `d45987f7` adds tests to `tests/soft-deleted.jl1.test.js`): `isValid: false` counts as deleted everywhere except v3 search and history; COB 40 g → none on both delete paths. Maintainer decision 2026-09-25: goes into 15.0.9 with BF-122 on the same branch. Found 2026-09-25 by the journey lab (JL-1). **Reproduced** on `v15.0.8` `92d08342` and `dev` `e3adc91d`: 40 g carbs → COB 40 → AndroidAPS v3 delete → COB still 40; the same through the AndroidAPS v1 socket `dbUpdate` |
| **BF-136** | **An AndroidAPS treatment that API v3 matches to a record written through API v1 is refused, and never reaches Nightscout.** A v3 POST with no `identifier` that finds an existing record with the same `created_at` and `eventType` becomes an update (deduplication), and the update validator compares fields a client may not change (among them `date`, `app`, `device` and `isValid`) with the stored record. A v1-born record has no `app`, `device` or `isValid`, and a careportal-shaped one has no `date` either, so the write is answered 400: on `date` for a careportal-shaped record, and "Field app cannot be modified by the client" when the v1 record carries a `date`. A v3 PUT of a v1-born record is refused on `date` whether or not `app` is sent. A v3 PATCH carrying `isValid: true`, which AndroidAPS sends on every update, is refused with "Field isValid", so every AndroidAPS edit of a v1-born record (for example a careportal temp target changed in AndroidAPS) was silently dropped. Deletes are not affected. AndroidAPS treats the 400 as final and advances its upload cursor (`NSClientV3Plugin.kt:1019`, `DataSyncSelectorV3.kt:256`, read), so it is never retried. (Corrected 2026-09-26: the row first said the refusal was always on `app`) | `lib/api3/generic/update/validate.js:21-43` (immutable fields `:21-22`, check `:39-42`); the dedup that turns the POST into an update, `lib/api3/generic/create/operation.js:43`, `:52`, with the treatments fallback `created_at` + `eventType` (`lib/api3/generic/setup.js:95`); identical on `v15.0.8` `92d08342` and `dev` `e3adc91d` | **safety** (dosing-relevant data loss in Nightscout): the AndroidAPS entry is missing from Nightscout's COB, IOB, chart, reports and what followers see, with no error on the site. The phone's own dosing is unaffected: AndroidAPS keeps its own record. Bounded to an AndroidAPS upload that lands on the `created_at` and `eventType` of a record written through v1 (for example the same meal entered in the careportal and on the phone at the same time), and to v3 edits of v1-born records | yes | **fixed 2026-09-26** on local branch `bf/api3-app-field-v1-records` `df6c04bc` (one commit on `dev` `e3adc91d`, not pushed): in `isSameAsStored` (`lib/api3/generic/update/validate.js`, used by POST dedup, PUT and PATCH) a field the stored record lacks is accepted when it contradicts nothing: `app` and `device` take their first value; `isValid` `true` is accepted (`false` stays refused); `date` only if it equals the stored `created_at` instant; `identifier`, `utcOffset`, `eventType` and the server fields stay strict. [Probe](../../../tools/lab/triage-2026-09/api3-app-field.js): exit 1 on `v15.0.8` `92d08342` and `dev` `e3adc91d` (3 of 13 arms), exit 0 on `df6c04bc` (13 of 13), 15 controls pass. 16 new tests (9 fail on `dev`), suite 3186/0/3; with BF-122's `718efddc` 3212/0/3. Trade-off, pinned by a test: v3 fallback dedup (`created_at` + `eventType`, amounts not compared) now replaces a same-millisecond v1 record, so careportal 20 g becomes AndroidAPS 30 g and the careportal `notes` and `enteredBy` are lost; before, the AndroidAPS entry was lost instead. Rare: careportal times are whole minutes. **Decision 2026-09-26 (maintainer):** fixed in 15.0.9. **Reproduced** 2026-09-25 on `v15.0.8` `92d08342` and `dev` `e3adc91d`: 20 g carbs as a v1 `Meal Bolus`, then the same time as a v3 `Meal Bolus` from app `AAPS` with no identifier: the v3 write is answered 400 and the stored record is the v1 one. The v3 PUT refusal was measured on both builds. The AndroidAPS side is read |
...
| **BF-138** | **`/pebble` asked for the units the site does not use computes the Bolus Wizard Preview against settings in the other units.** On an mg/dL site asked for `?units=mmol`, `prepareSandbox` switches the sandbox to mmol/L while the profile's sensitivity and targets stay in mg/dL, so `bwp` and `bwpo` compare an mmol/L reading with mg/dL settings. On a booted server the usual wrong output on an mg/dL site asked for mmol is `bwp` −0.96 (right by coincidence) with `bwpo` 23.2, an mg/dL number in an mmol response, because of BF-139; `bwp` −2.17 and `bwpo` −61.8 (site values −0.96 and 23; sensitivity 70, targets 90-126, 1 U given 30 min before) appear only in-process on fresh data, or between a data load and its evaluation. On an mmol/L site asked for mg/dL, `bwpo` stays in mmol/L (1.3) next to an mg/dL reading, unlabelled | `lib/server/pebble.js` `prepareSandbox` `:143-154` (mmol switch `:146-148`), BWP fields `:95-99`; `lib/plugins/boluswizardpreview.js`; the same on `v15.0.8` `92d08342` and `dev` `e3adc91d` | **safety** (dosing-relevant display on a watch face): the bolus wizard preview a watch face shows is wrong in size and sign, with nothing marking it. Bounded to clients that ask `/pebble` for the units the site does not use | yes | **fixed 2026-09-26** on local branch `bf/pebble-delta-units` `b06eb014` (after `df149642`, BF-139, on `aa224c69`, BF-128; one PR; not pushed): `iob`, `cob` and `bwp` are computed in a sandbox in the site's units; `bwpo` is converted to the requested units; `bwp` is not converted. Of 18 combinations of site units and request, only `bwp`/`bwpo` change, in the two mismatched ones (plus BF-128's delta). Suite on `b06eb014` 3216/0/3 (25 new: 4 BF-139, 21 BF-138). Found 2026-09-25 while fixing BF-128. **Decision 2026-09-26 (maintainer):** fixed in 15.0.9 with BF-128, on `bf/pebble-delta-units`, one PR. **Reproduced** in-process 2026-09-25 on `v15.0.8` and `dev`, and on a booted server 2026-09-26 (corrected figures in this row) |
| **BF-139** | **A `/pebble` request can change the units of glucose values other requests read.** The sandbox a `/pebble` request builds shares the server's reading objects: `ddata.clone()` is a shallow copy, and `sbx.scaleEntry` (called by `bgnow` for each reading) writes `entry.scaled` onto the reading when it is not already set. A request in units other than the site's can therefore leave readings scaled in its units, and until the next data load other readers of the same objects can see values in the wrong units. It reaches the server's own alarm evaluation (reproduced on a booted server 2026-09-26); detail withheld pending the maintainer's disclosure decision. Described by mechanism only | `lib/data/ddata.js` `clone` `:109-111` (shallow); `lib/sandbox.js` `scaleEntry` `:258-268`; `lib/plugins/bgnow.js:85`; `lib/server/pebble.js` `prepareSandbox` `:143-154`; the same on `v15.0.8` `92d08342` and `dev` `e3adc91d` (read) | **safety** (alarm integrity), raised 2026-09-26 from correctness: glucose values in the wrong units on other readers until the next data load, and it reaches the server's own alarm evaluation; detail withheld pending the maintainer's disclosure decision | yes | **fixed 2026-09-26** on local branch `bf/pebble-delta-units` `df149642` (on `aa224c69`, BF-128; not pushed): the `/pebble` sandbox gets its own copies of the readings without a stored scaled value. Noticed 2026-09-25 while fixing BF-128. **Decision 2026-09-26 (maintainer):** fixed in 15.0.9 with BF-128, on `bf/pebble-delta-units`, one PR. **Reproduced** on a booted server 2026-09-26: it reaches the server's own alarm evaluation; detail withheld pending the maintainer's disclosure decision |
...
**2026-09-26, from BF-121's fix.** `bf/same-time-treatments` `fc821024` makes the socket's ±2 s
similar match always key on `eventType`, which fixes this entry's X1 and X2 cases (a zero temp
matching another `eventType`). Z2, Z5, P2, B2 and C2 remain. A BF-09 branch will conflict with
...
**Addendum, measured 2026-09-25** on `v15.0.8` `92d08342` and on `bf/entries-unknown-id` `f79dc732`
(Node 22.23.2, MongoDB 7.0.43), while measuring BF-129. A second route to the same kind of
disclosure needs no crafted input: while MongoDB is unreachable, any read that reaches the database
answers a JSON 500 whose body carries the driver's error with its stack. It is not the HTML
`errorhandler` page, and the condition is an outage the requester does not control, but it
qualifies the scope paragraph above: that paragraph's seven probes were run with the database up.
The remedy is the same in kind (log the error server-side, answer a bare 500).
...
**Re-measured 2026-09-25 on `dev` `e3adc91d`** (which carries the BF-75 fix and #8754; mongod
7.0.43, Node 22.23.2, 75 runs), timing **alarm receipt** rather than admission. The figures in the
table above are admission times, not the time the alarm arrived.

- **Who is held back.** A device with valid web credentials (a JWT or the API secret), or an
  anonymous viewer on a readable site, that connects or reconnects to `/alarm` while its resolved
  address is in the failed-login delay. It is admitted only after that delay: about 5·(N−1) s
  after N overlapping failures, about 5 s for serial retries.
- **What it costs.** The socket misses the alarm as first emitted and receives it at the next
  re-emission (the heartbeat or the next reading): **45-56 s late for N = 1 to 10**, and **81.7 s**
  under `AUTH_DEFAULT_ROLES=denied` with N = 10.
- **Why valid credentials do not help.** The wait comes before the credential check
  (`lib/authorization/index.js:152-158`, deliberately), so a successful authentication does not
  shorten it.
- **Not affected.** Sockets from a different resolved address; sockets already in the room;
  `subscribe({accessToken})`, the path AndroidAPS uses (read, not run).
- **What counts as a failure.** A bad `Authorization: Bearer` token, a bad `api-secret` header, or
  bad socket credentials. A bad `?token=` query token does not count.
- **What anyone sees.** Each failure raises an admin-only "Failed authentication" notification
  naming the address. Nothing says that alarms are being held back.
- **Control.** On `v15.0.8` every scenario delivered the alarm in 0-2 ms, because the broadcast
  there checks nothing (BF-75).
...
control and that belongs in its own change with its own review. PR #8754 (in `dev` since
2026-09-25, merge `4f705217`) keeps the wait before the credential check; the 2026-09-25
measurement above shows that devices with valid credentials are held back too, so option 1 alone
would not cover every affected viewer.

**Decision 2026-09-26 (maintainer): option 1.** A viewer that presents no credential is not held by
the failed-login delay. (The 2026-09-26 decision round numbered this option 2; the numbers here are
this section's.) The maintainer's reasoning: an anonymous viewer on a site that is readable should
be fine, and leaving it as it is would also have been acceptable, because a device that keeps
failing its logins is likely to be noticed and fixed. Options 2 and 3 were not chosen.

- A viewer that presents a credential (a JWT, the API secret, or socket credentials) keeps the
  delay, unchanged. A signed-in page or a device with valid credentials, behind an address in the
  delay, still receives an alarm late: 45-56 s in the 2026-09-25 measurement above.
- It narrows a brute-force control, so it goes in its own PR with its own review.

**Fixed 2026-09-26** on local branch `bf/alarm-anonymous-no-delay` `8e295769` (worktree
`externals/work/crm-r2-fix-80`; one commit on `dev` `e3adc91d`, not pushed; Node 22.23.2, MongoDB
7.0.43):

- A `/alarm` subscribe that carries no credential (the secret, `jwtToken`, `token` and
  `accessToken` all absent, null or empty) is answered with the site's `AUTH_DEFAULT_ROLES`
  without the failed-login delay, through a new `authorization.resolveAnonymous`. `resolve()` and
  every HTTP path are unchanged, and #8754's test that an anonymous request waits still passes.
- **Measured on a booted server** with the default delay, after 3 failed logins from the viewer's
  address: an anonymous viewer on a readable site went from an ack at 9999 ms and no alarm within
  20 s (`e3adc91d`) to an ack at 1 ms and the alarm at 13 ms. An anonymous viewer on a denied site
  is answered in 1 ms with `read: false` and still receives no alarm, so BF-75's fix holds.
- Every subscribe that carries a credential, valid or not, is unchanged: ack after about 10 s,
  alarm at the next repeat (37.7 s on a readable site, 49.5 s on a denied one).
- Suite 3180/0/3; 10 new tests (`tests/api3.alarm-socket.anonymous-delay.test.js`), 4 of which
  fail on `e3adc91d`.
- **Still waiting after the fix:** a signed-in viewer on an address in the delay; a socket that
  connects to `/alarm` without subscribing; chart data on the main namespace. Making the
  connect-time admission skip the delay for a no-credential socket too is a one-hunk change,
  offered to the maintainer and not made.
...
[the advisory response pack](./advisory-response-2026-09/README.md).*
...
candidate sinks ([evidence](../../60-research/remedial/connector-profile-sync.md)).
...
**Correction 2026-09-25: a delete is affected too.** The row first said no write or delete was
affected. That was wrong. On `dev` a bulk `DELETE /api/v1/devicestatus` with a `find[sgv]` bound
deletes nothing where 15.0.8 deleted the matching records, and a devicestatus `sgv` read filter has
regressed in the same way as activity's.

**Where it started.** The `SHIPPING_WALKERS` table in `tools/nsschema/emit/coercion_emit.py`
(the `devicestatus` and `activity` entries, about lines 161-168) records `"walker": {}` for both
collections. On 15.0.8 neither module set a walker, so both inherited the `{date, sgv}` default;
only `profile` set `walker: {}` (`lib/server/profile.js:97` on `v15.0.8`). That mis-transcription
is where BF-106 started. The emitter is not changed by the fix.

**Fixed 2026-09-25** on local branch `bf/activity-date-coercion` `20c197bb` (one commit on
`origin/dev` `e3adc91d`, not pushed; worktree `externals/work/crm-6a-fix-activity-date-coercion`).
In `lib/server/query.js`, when a collection is named and no walker is given, the default is
`legacyNumericDefaults(collection)`: `date` and `sgv` read as numbers through
`coercion.toNumber`, and only for a field the collection's schema does not type. A field the
schema types keeps the schema's type; other fields stay as they arrived. `lib/server/profile.js`
sets `walker: {}` again, as on 15.0.8. This restores the activity `date` and `sgv` filters and the
devicestatus `sgv` filter, including the bulk DELETE. The gate
`tools/queue/gates/bf106-activity-date-coercion.js` exits 1 on `e3adc91d` and 0 with
`--ref bf/activity-date-coercion`. 18 new tests (`tests/api.activity-date-coercion.test.js`, a
group in `tests/query.test.js`); full suite 3188/0/3 (Node 22.23.2, MongoDB 7.0.43).

No client in the survey filters activity by `date`, which bounds the impact; the survey reads
client source and cannot see filters built at runtime.
...
**Fix (`6c3ccce6`, local):** each write matched by `_id` is followed directly by the delete of
that id's string forms, so a later item matched by time and type finds the one record and replaces
it, which is what a batch of two items with the same `created_at` and `eventType` does when no
string copy exists (the control test). The upserted ids are mapped back to their items past the
added deletes. Tests in `tests/api.object-id.treatments-entries.test.js`: the reproduction (red on
...
### BF-133 · the COB pill's "last carbs" can name an older carb entry

`lib/plugins/cob.js` walks the treatments it is given and sets `lastCarbs` to each carb treatment
in turn, so `lastCarbs` is the last one in array order. The treatments array is usually sorted by
time, but not always after an edit. The soak (run R13 and dumps at ticks 45 to 63) saw `lastCarbs`
move from the newest carb entry (15 g) to an entry about 3 hours older (25 g) after that older entry
was edited, on 15.0.8, `dev`, #8758 `ab7b22d6` and `e9dbb1fb` alike; `ab7b22d6` switched one reload
earlier, which is the one A/B difference the soak flagged. The COB total is computed from all carb
entries and is unaffected.

**Fix shape, not measured:** pick the carb treatment with the greatest `mills` rather than the
last one seen; find out why the array order changes after an edit.
...
stored under it, the route sets `res.entries_err = "No such id: '<spec>'"` and continues, and the
formatter answers a set `entries_err` with HTTP 500. Every other v1 read that matches nothing
answers 200 with `[]`. #8758 (`a612a26f`) makes `isId` accept upper-case hex, as every other route
already did, so an upper-case id that 15.0.8 treated as a `spec` (200, `[]`) now reaches the 500.

**Measured 2026-09-25** by the #8758 freeze review on `v15.0.8` `92d08342` and `ab7b22d6`: 500 on
both for a lower-case unknown id; for an upper-case unknown id 200 `[]` on 15.0.8 and 500 on
`ab7b22d6`.

**Measured 2026-09-25** (Node 22.23.2, MongoDB 7.0.43):

| unknown 24-hex id | `v15.0.8` `92d08342` | `dev` `e3adc91d` | `bf/entries-unknown-id` `f79dc732` |
|---|---|---|---|
| lower-case | 500 | 500 | 200 `[]` |
| upper-case | 200 `[]` | 500 | 200 `[]` |

On 15.0.8 an entry stored under a lower-case hex *string* `_id` also answered 500 when fetched by
its id; #8758 fixed that. The consumer survey finds no corpus client that fetches an entry by id.

**Fixed 2026-09-25** on local branch `bf/entries-unknown-id` `f79dc732` (one commit on
`origin/dev` `e3adc91d`, not pushed; worktree `externals/work/crm-6a-fix-entries-unknown-id`).
Decided 200 `[]` over 404: when `getEntry` finds nothing the route answers 200 with `[]`, the
answer every other v1 read (including `find[_id]=<id>`) gives when nothing matches. A storage
error still takes `next(err)` and answers 500. The swagger description of the `spec` parameter
says so. 11 new tests (`tests/api.entries.unknown-id.test.js`); full suite 3181/0/3.

While measuring this, a second route to BF-73's kind of disclosure was found; it is recorded in
BF-73's addendum.
...
**Design measured 2026-09-25** on `dev` `e3adc91d` with a lab prototype, `lab/6a-fix-121`
`a8ca1798` (worktree `externals/work/crm-6a-fix-same-time-treatments`, local, not for pushing),
which switches between three fallback keys. Identity-aware matching fixes the Loop, v3-then-v1 and
similar-match arms, with no re-send regression across 19 client shapes. Adding the amounts to the
key fixes the remaining arms, but makes an AndroidAPS edit after a lost id a duplicate on v3 and on
the socket.

**Does it occur? Yes.** Issue #8185 is a real report: a caregiver sent Loop remote carbs of 16 g
and then 4 g, both back-dated to 7:40. The 16 g entry disappeared from Nightscout's chart, reports
and daily total, while Loop's own carbs on board kept 20 g.

**Decision 2026-09-26 (maintainer): option 3 of the measured design.**

- Identity-aware matching (the prototype's `safe` mode) for v1 writes and the websocket: a
  client's own id (`syncIdentifier`, `id`, `uuid`, `NSCLIENT_ID`) takes part in the match, and a
  write without one does not replace a record that has one.
- In addition, the carbs and insulin amounts join the `created_at` + `eventType` key, **only for
  v1 REST writes that carry no client identity**.
- AndroidAPS v3 writes and the websocket `dbAdd` get no amounts in their key, so the duplicate the
  full design made of an AndroidAPS edit after a lost id does not arise.

**Still stored as one after the fix (known issues, decided):**

- AndroidAPS through API v3: a bolus and carbs uploaded in the same millisecond, both with
  `eventType` `Meal Bolus`, get the same computed identifier and are stored as one.
- The careportal: two entries of the same type at the same minute with the same amount. Nothing in
  the record tells them apart from a resend, and no key design separates them.

**Fixed 2026-09-26** on local branch `bf/same-time-treatments` `fc821024` (worktree
`externals/work/crm-r2-fix-121`; one commit on `dev` `e3adc91d`, not pushed; Node 22.23.2, MongoDB
7.0.43, full suites on a fresh database). The key lives in `lib/server/treatment-fallback-key.js`
(new), used by `lib/server/treatments.js` and `lib/server/websocket.js`.

- **Rule.** The client's own identity (`syncIdentifier`, `id`, `uuid`, `NSCLIENT_ID`) must match,
  and a write without one matches only a record without one (v1 and the socket `dbAdd`). The carbs
  and insulin amounts join the `created_at` + `eventType` key for v1 writes without identity. The
  socket's ±2 s similar match always keys on `eventType`.
- **Fixed arms and cases.** v1-single, v1-array, v1-careportal, v3-then-v1 and ws-similar; the
  tconnectsync bolus plus extended bolus, careportal-then-Loop, and careportal 20 g then 15 g cases.
- **No re-send regression.** Every one of 22 measured re-send shapes stores what it stored before
  ([`same-time-resend-shapes.js`](../../../tools/lab/triage-2026-09/same-time-resend-shapes.js), new).
- **Left by design.** v3 without `identifier`; the socket `dbAdd` without identity, where an edit
  sent after a lost acknowledgement stays stale; an AndroidAPS v3 bolus and carbs in the same
  millisecond; and careportal entries at the same minute with the same amount, which cannot be
  told apart from a re-send.
- **Tests.** 18 new tests (`tests/api.same-time-treatments.test.js` and two changed files), 10 of
  which fail on `e3adc91d`; suite 3189/0/3.
- **Probe.** [`same-time-carbs.js`](../../../tools/lab/triage-2026-09/same-time-carbs.js) now prints
  a verdict per arm. The arms left by design (v3-noid, ws-dbAdd) are reported as known issues and
  do not decide the exit status; `--strict` counts them. It exits 1 on `e3adc91d` and 0 on
  `fc821024`.
- **Conflict with BF-122's branch.** `bf/same-time-treatments` conflicts textually with
  `bf/v1-writes-v3-history` (the requires at the top of `websocket.js` and one expectation in
  `tests/websocket.input-validation.test.js`). Resolved in a trial merge: 3215/0/3. BF-122's
  branch is to merge first.
- **Interaction with BF-09.** BF-09's X1 and X2 cases (a zero temp matching another `eventType`
  within ±2 s) are fixed by this change, because the similar match now keys on `eventType`. Z2,
  Z5, P2, B2 and C2 remain. A BF-09 branch will conflict with this one on the similar-match lines.

**Question for the maintainer (not decided).** With BF-136's fix and this one both present, an
AndroidAPS v3 POST at the same `created_at` and `eventType` as a v1 record takes that record over
(it gains an `identifier`, by BF-136's rule). A later identical v1 re-send of the original record
then no longer matches, because it carries no identity and the record now has one, so it is
stored as a second record when the amounts are equal (with different amounts both were kept
anyway). It needs all three conditions: the v1 record, the AndroidAPS write at the same instant
and type, and a later v1 re-send. No client in the corpus was found that does all three.
...
**Partly fixed 2026-09-25 (creates and updates)** on local branch `bf/v1-writes-v3-history`
`718efddc` (one commit on `origin/dev` `e3adc91d`, not pushed; worktree
`externals/work/crm-6a-fix-v1-writes-v3-history`). The same commit fixes BF-135.

- Every v1, websocket and in-process write stamps `srvModified`, and `srvCreated` on insert, for
  every collection API v3 serves (`lib/server/srv-dates.js`).
- A v1 PUT or `replaceOne` reads the record it replaces and keeps its `identifier` and
  `srvCreated`, so a v3 record changed through v1 stays the record a v3 client knows.
- Every writer, v3 included, takes `srvModified` from one server clock that never repeats or goes
  back in the process.
- The probe now finds the treatments, entries and devicestatus v1 arms and the v1 PUT arm in
  history. It still exits 1, only on the v1 DELETE arm: a v1 DELETE (and websocket `dbRemove`)
  still removes the record outright, so history has nothing to return.
- **The delete design was decided on 2026-09-26 (below).** The options were: (a) keep hard delete;
  (b) soft delete everywhere; (c) soft delete for single-record deletes; (d) hard delete plus a
  minimal tombstone.
- 26 new tests (`tests/api3.v1-writes-history.test.js`; 20 fail on `e3adc91d` with the original
  symptom), 24 break-it checks, full suite 3196/0/3 (Node 22.23.2, MongoDB 7.0.43). Three existing
  expectations changed, each marked in its file: two dedup tests in `tests/api3.create.test.js`,
  the dedup selectors in `tests/websocket.input-validation.test.js`, and the fake collection in
  `tests/storage.selector-hardening.test.js`.
- **Release note.** Careportal and caregiver carbs now reach AndroidAPS through v3 history. The
  same meal entered in both places may then appear twice on the phone.
- **Found while fixing, not filed.** v3 history can skip a write whose commit lands after a
  later-stamped write has been read: the stamp is taken before the commit, so there is a
  millisecond window. It affects v3 writes too, and is for after 15.0.9.

**Decision 2026-09-26 (maintainer): v1 DELETE keeps hard delete (option a).** The reason given:
people are likely to use AndroidAPS as the controller, not the careportal. AndroidAPS deletes are
soft (`isValid: false`); with BF-135's fix they stop counting on the site, and they reach other
AndroidAPS instances through v3 history. Consequences:

- A delete made through v1 or the websocket `dbRemove` (the careportal, Loop, Trio, xDrip+) still
  removes the record outright and never appears in v3 history. AndroidAPS keeps its copy and keeps
  counting it. The record has to be deleted in AndroidAPS as well.
- The Admin Tools and API purges keep freeing space, and no tombstones accumulate.
- The probe's v1 DELETE arm now measures the decision, not a pending defect: the record is
  expected to be absent from history, and the probe exits 1 on that arm alone. The branch's test
  "a v1 DELETE still removes the record, and history does not report it" pins it; since
  `dbc4c5fc` its name and comment record the decision (hard delete kept).
- What AndroidAPS takes from history depends on its own settings (AndroidAPS `7e1d537d49`, read).
  On a looping phone, carbs and insulin from Nightscout, and their deletions, are applied only when
  NSClient's "accept carbs" and "accept insulin" settings (`ns_receive_carbs`,
  `ns_receive_insulin`) are on; both default to off (`core/keys/.../BooleanKey.kt:224-225`). The
  AAPSClient (follower) build always applies them and hides the two settings
  (`showInNsClientMode = false`; `NsIncomingDataProcessor.kt:163-167`,
  `StoreDataForDbImpl.kt:420-426`). A full sync applies them too.

**Branch head `d45987f7`** (2026-09-26): one test commit on `718efddc`, touching only
`tests/soft-deleted.jl1.test.js`: a v1 profile search by date leaves out a deleted profile with
the same date, and returns it when `find[isValid]=false` is asked for. 30 new tests across the two
files; full suite 3200/0/3 (Node 22.23.2, MongoDB 7.0.43).

**Branch head `dbc4c5fc`** (2026-09-26): renames the pinned v1 DELETE test and its comment to
record the decision; no other change, so the suite figure on `d45987f7` stands.
...
`tests/profile-switch-preprocessing.test.js`. *(Superseded by the fix below, which applies
`timeshift` the way AndroidAPS 3.x does.)*

**Fixed 2026-09-25** on local branch `bf/profile-switch-percentage` `5a895b49` (one commit on
`origin/dev` `e3adc91d`, not pushed; worktree `externals/work/crm-6a-fix-profile-switch-percentage`).
A switch of the AndroidAPS 3.x shape (an embedded `profileJson`, a numeric `percentage`, `timeshift`
in milliseconds, no `CircadianPercentageProfile`) is read as AndroidAPS reads it: basal ×
pct/100, ISF and carb ratio × 100/pct, targets and DIA unscaled, and every schedule looked up at
t − (timeshift truncated to whole hours), wrapped to one day (AndroidAPS `7e1d537d49`,
`BlockExtension.kt:14-18`, `ProfileSealed.kt:77`, `:324-365`). Requests for a named profile are
not adjusted. The probe exits 1 on `e3adc91d` and 0 on the branch; 35 new tests
(`tests/profile-switch-percentage.test.js`); full suite 3205/0/3 (Node 22.23.2, MongoDB 7.0.43).
Not checked in a browser (the chart's basal line, the reports).

**Left for the maintainer: the `CircadianPercentageProfile` path**, which is unchanged. Its
intended timeshift direction (t + offset, AAPS 2.x) is the opposite of AAPS 3.x's (reversed in
AndroidAPS `37e3c4532a`), and, as measured above, it never applies the shift anyway.
**Decision 2026-09-26 (maintainer):** leave it as it is; it is not a defect to fix.
...
**Half 1 fixed 2026-09-25** on local branch `bf/maker-level-names` `2f50ada9` (one commit on
`origin/dev` `e3adc91d`, not pushed; worktree `externals/work/crm-6a-fix-maker-level-names`). A new
`levels.toKey` returns the untranslated lower-case level (`urgent`, `warning`, `info`, `low`,
`lowest`, `none`, `unknown`), and `sendMakerEvent` names the event from it, so `en`, `ru` and `de`
sites send identical event names; `value1`/`value2` keep the translated title and message. The
probe exits 1 on `e3adc91d` and 0 on the branch. 11 new tests (`tests/maker-level-names.test.js`);
full suite 3181/0/3 (Node 22.23.2, MongoDB 7.0.43).

**Half 2 kept by decision.** The 2015 commits `f805633f` and `ea745065` say a failed send is
retried, which is what the 30-second key does. The branch's tests now pin that behaviour (a failing
stub is retried), so a change to it is a visible decision. **Decision 2026-09-26 (maintainer):**
keep it. A failed Maker call keeps being retried about every 30 s, so the entry is recorded fixed.
With more than one Maker key, the retry does not cover a key that fails while another succeeds
(BF-137).

**Release note needed.** A site that renamed its IFTTT applets to the translated event names must
rename them back to `ns-warning`, `ns-urgent` and `ns-<level>-<plugin>`.
...
**Fix shape first written here, now known to be unsafe:** in `prepareSandbox`, set
`clonedEnv.settings.units = req.mmol ? 'mmol' : 'mg/dl'`. It fixes the delta, but the Bolus Wizard
Preview fields `/pebble` adds then compare a mg/dL reading with the site's mmol/L profile: on an
mmol/L site asked for mg/dL it gives `bwp` `"20.32"` where the site's own value is `"-0.96"`
(measured 2026-09-25). Do not use it.

**Fixed 2026-09-25** on local branch `bf/pebble-delta-units` `aa224c69` (one commit on
`origin/dev` `e3adc91d`, not pushed; worktree `externals/work/crm-6a-fix-pebble-delta-units`).
`addDelta` takes `delta.mgdl` unless mmol is requested, and `delta.scaled` (mmol, as before) when
it is. The sandbox units are not changed, so `bwp`, `bwpo`, `iob` and `cob` are computed exactly as
before. The probe exits 1 on `e3adc91d` and 0 on the branch. 21 new tests
(`tests/pebble-units.test.js`: site mg/dL or mmol × no units, `?units=mgdl`, `?units=mmol` ×
rising, falling, flat; the legacy 0 with no delta; the BWP, IOB and COB fields unchanged by
`?units` on an mmol site); full suite 3191/0/3 (Node 22.23.2, MongoDB 7.0.43).

**Filed 2026-09-26.** Two further observations from the fix are now entries of their own:

- **BF-138**: on an mg/dL site asked for `?units=mmol`, `/pebble` computes the Bolus Wizard Preview
  in an mmol sandbox against the mg/dL profile (`bwp` `-2.17` where the site's own value is
  `-0.96`). `aa224c69` does not change this.
- **BF-139**: a shared-state mechanism, in which the values one `/pebble` request scales can
  persist on data shared with later requests. Mechanism only.

Maintainer decision 2026-09-26: both are fixed in 15.0.9 on this branch, in one PR with BF-128.
Both are fixed on the branch (`df149642` BF-139, then `b06eb014` BF-138; see their sections). One
of `aa224c69`'s tests expected `bwpo` to be unchanged by `?units` on an mmol site; with BF-138's
fix `bwpo` follows `?units=mgdl`, so that expectation was changed by design. Suite on `b06eb014`
3216/0/3.

### BF-132 · on #8730, the Crowdin sync reverts translations dev corrected

#8730 (`crowdin_incoming`, head `f99c0e54`, opened 2026-09-21 by the Crowdin integration) carries
Crowdin's current strings. Dev's `9f249815` corrected several of them in the repository, and
Crowdin was not updated, so merging #8730 puts the older strings back.

**Reproduced 2026-09-25** in the release-candidate pre-flight (dev `ecb63223` + #8758 and its fixes
+ #8568 + #8419 + #8730, tree `dd51bb8a`): four tests in `tests/language.test.js` fail in all six
Node × MongoDB cells and pass on dev: `expected '克' to be '毫升'` (zh_TW "ml"), `expected '碳水' to be
'碳水化合物'` (zh_TW "Carbs"), `expected 'Days with food' to be 'Dagar med mat'` (sv_SE), and the Russian
status-line substitution. Read from the files: `git show refs/triage/pr-8730:translations/zh_TW.json`
has "ml": "克"; dev has "毫升".

**Fix shape (not measured):** correct the strings in Crowdin to match dev, then let Crowdin
refresh #8730, or close it and take the next sync. A commit pushed to `crowdin_incoming` would be
overwritten by the next sync.

### BF-134 · every Loop remote command leaves an APNs connection open

`lib/server/loop.js` `sendNotification` built `new apn.Provider(options)` near its start and
called `provider.send(...)` at the end, without ever calling `provider.shutdown()`. In
`@parse/node-apn` 5.2.3 a Provider's Client opens an HTTP/2 session to APNs on first send and
keeps it open with a heartbeat ping every 60 s (`config.heartBeat` 60000, an `unref`'d
`setInterval` in `lib/client.js`), until `shutdown()` clears both. On a server GOAWAY the library
...
leaves 1 session; an explicit `shutdown()` leaves 0. A remote bolus that `loop.js` refuses after
its checks still built a provider (1 interval). With no Loop settings at all, the call throws
before building one. The same probe exits 0 on `bf/loop-apns-shutdown`. Not run: real APNs, and a
long-running process's memory (session -6a's soak saw a higher resident size on the candidate, one
sample, not attributed).

**Found** from #8419's new test hang: `fakeAPNServer.close()` waits for open sessions; Node 24
closes idle ones itself, 20 and 22 do not.

**Fix (on `bf/loop-apns-shutdown`, `8046336d`):** build the provider just before the send and call
`provider.shutdown()` in a `finally` on the send; refused commands no longer build one.
`tests/loop-apns-connections.test.js` counts open sessions after three commands, after a refused
push, and providers built for a refused bolus; all three fail on dev's `loop.js`. Full suite
3120/0/3 vs dev 3117/0/3 (Node 22.23.2, MongoDB 7.0.43).

### BF-135 · a record AndroidAPS deletes keeps counting in COB, IOB and the chart

**Found 2026-09-25 by the journey lab** as JL-1
([journey lab §JL-1](../../60-research/remedial/journey-lab-15.0.9-2026-09-25.md),
[journey map §B5](../../60-research/remedial/journey-map-15.0.9.md)).

**Mechanism.** AndroidAPS deletes by soft delete. Through API v3 it sends a DELETE, and the server
sets `isValid: false` on the record; through its v1 client it sends a websocket `dbUpdate` that
sets `isValid: false`. API v3 search and history honour the flag. Nothing else does: v1 reads
still return the record, and the data loader, and with it COB, IOB and the chart, the server
cache, `/api/v1/profile/current` and `/api/v2/properties`, keep counting it. Outside API v3, the
only `isValid` check is for `OpenAPS Offline` (`lib/data/ddata.js:351`, the same on `v15.0.8` and
`dev` `e3adc91d`). Any API v3 client's DELETE is a soft delete, so the same holds for it.

**Reproduced** on `v15.0.8` `92d08342` and `dev` `e3adc91d` in the journey lab, on a site with no
device status so that COB comes from treatments: AndroidAPS-shaped carbs of 40 g (v3 POST) give
COB 40; after the AndroidAPS v3 delete, COB is still 40. The same happens through the AndroidAPS
v1 socket `dbUpdate`. Control (run on `e3adc91d` only): a v1 DELETE of the same record by `_id`
leaves no COB. It exists on
15.0.8, so it is not a 15.0.9 regression. IOB was covered by the fix's tests; the chart glyph and
the report totals were read, not run.

**Severity.** Safety, dosing-relevant display: Nightscout's COB and IOB, and what it hands to
followers, are overstated by the deleted amount, with no error. Check COB and IOB in the app that
doses. This is not medical advice.

**Decision 2026-09-25 (maintainer):** fixed in 15.0.9, with BF-122, on the same branch.

**Fixed 2026-09-25** on local branch `bf/v1-writes-v3-history` `718efddc` (the BF-122 commit; one
commit on `origin/dev` `e3adc91d`, not pushed). `lib/server/soft-deleted.js`: a record with
`isValid: false` counts as deleted everywhere except v3 search and history: v1 list, count and
single-entry reads, `profile/current`, food lists, the data loader's loads (COB, IOB, the chart,
the websocket data), the cache (an update to `isValid: false` evicts the record), and the
websocket `dbAdd` dedup lookups. A read that names `isValid` in `find` gets what it asks for. A v1
DELETE still removes soft-deleted records. An entry re-sent for a deleted reading is stored
again, as a treatment and a v3 POST are. COB goes from 40 to none on both delete paths
(`tests/soft-deleted.jl1.test.js`, which also covers IOB, a deleted profile, glucose reading,
device status and food, and the cache).

### BF-136 · API v3 refuses an AndroidAPS write that lands on a record written through v1

**Mechanism.** A v3 POST with no `identifier` looks for an existing record by the collection's
fallback fields (treatments: `created_at` + `eventType`, `lib/api3/generic/setup.js:95`) and, when
it finds one, becomes an update of that record (`lib/api3/generic/create/operation.js:43`, `:52`,
`isDeduplication: true`). The update validator (`lib/api3/generic/update/validate.js:21-43`) lists
`identifier`, `date`, `utcOffset`, `eventType`, `device`, `app`, `srvCreated`, `subject`,
`srvModified`, `modifiedBy` and `isValid` as fields a client may not change, and exempts only
`identifier` during deduplication. A record written through v1 has no `app`, `device` or
`isValid`; a careportal-shaped one has no `date` either. An AndroidAPS record has them, so the
comparison fails and the POST is answered 400.

**Corrected 2026-09-26** (measured while fixing; the first version of this section said the
refusal was always on `app`):

- A careportal-shaped v1 record is refused on `date`, because careportal records have no `date`.
  The refusal is on `app` ("Field app cannot be modified by the client") only when the v1 record
  carries a `date`.
- A v3 PUT of a v1-born record is refused on `date`, whether or not `app` is sent.
- A v3 PATCH carrying `isValid: true`, which AndroidAPS sends on every update, is refused with
  "Field isValid". So every AndroidAPS edit of a v1-born record was silently dropped, for example a
  careportal temp target that AndroidAPS changes.
- Deletes are not affected.

**What AndroidAPS does with the 400** (AndroidAPS `7e1d537d49`, read): `NSClientV3Plugin.kt:1019`
logs "◄ FAIL" and falls through, and `DataSyncSelectorV3.kt:256` advances the upload cursor, so
AndroidAPS treats the 400 as final and does not retry the record (a 404 is retried; a 400 is not).
The entry stays on the phone and never reaches Nightscout.

**Reproduced 2026-09-25** on `v15.0.8` `92d08342` and `dev` `e3adc91d`: a v1 treatment
(`Meal Bolus`, 20 g carbs) at time T, then a v3 `Meal Bolus` at T from app `AAPS` with no
identifier. The v3 write was answered 400 and the stored record stayed the v1 one. The v3 PUT
refusal was measured on both builds (also recorded under OID-V3-EDIT-MERGE).

**Severity.** Safety, dosing-relevant data loss in Nightscout: the AndroidAPS entry is missing from
Nightscout's COB, IOB, chart, reports and what followers see, and nothing on the site says so. The
phone's own dosing is unaffected, because AndroidAPS keeps its own record. It needs the same
`created_at` and `eventType` as a record written through v1, for example the same meal entered in
the careportal and on the phone for the same time. BF-122's fix, which sends careportal entries to
AndroidAPS, makes it more likely that both exist.

**Decision 2026-09-26 (maintainer):** fixed in 15.0.9.

**Fixed 2026-09-26** on local branch `bf/api3-app-field-v1-records` `df6c04bc` (worktree
`externals/work/crm-r2-fix-136`; one commit on `dev` `e3adc91d`, not pushed; Node 22.23.2, MongoDB
7.0.43):

- **Rule** (`isSameAsStored` in `lib/api3/generic/update/validate.js`, used by the POST dedup, PUT
  and PATCH): a field the stored record lacks is accepted when it contradicts nothing. `app` and
  `device` take their first value; `isValid: true` is accepted and `false` stays refused; `date`
  is accepted only if it equals the stored `created_at` instant. `identifier`, `utcOffset`,
  `eventType` and the server fields stay strict.
- **Probe** [`api3-app-field.js`](../../../tools/lab/triage-2026-09/api3-app-field.js): exit 1 on
  `v15.0.8` `92d08342` and `dev` `e3adc91d` (3 of 13 arms refused), exit 0 on `df6c04bc` (13 of
  13); its 15 controls pass on all three.
- **Tests.** 16 new tests (`tests/api3.v1-record-immutable-fields.test.js`), 9 of which fail on
  `dev`; suite 3186/0/3, and 3212/0/3 merged with BF-122's `718efddc`.
- **Trade-off, pinned by a test.** With the fix, the v3 fallback dedup (`created_at` +
  `eventType`, amounts not compared) replaces a v1 record at the same millisecond: careportal
  20 g becomes the AndroidAPS 30 g, and the careportal `notes` and `enteredBy` are lost. Before the
  fix the AndroidAPS entry was lost instead. Rare, because careportal times are whole minutes.
  With BF-121's fix as well, this takeover raises a question for the maintainer (BF-121's detail).

### BF-137 · several IFTTT Maker keys break the order of an alarm's calls and the retry

**Mechanism.** `MAKER_KEY` (and `MAKER_ANNOUNCEMENT_KEY`) may hold several keys separated by spaces
(`lib/plugins/maker.js:10-14`). `makeRequests` (`:82-97`) sends three events per alarm, `ns-event`,
`ns-<level>` and `ns-<level>-<plugin>`, as three steps of `async.series`, which expects each step
to call back once. `makeKeyRequests` (`:98-104`) calls `makeKeyRequest` for every key with the same
callback, so a step with N keys calls back N times. In `async` 0.9.2 (the version `package.json`
pins with `^0.9.2`), each extra callback starts the next step again and calls the final callback
again.

The final callback is `sendMakerEvent`'s (`lib/server/pushnotify.js:124-144`). A success extends
the alarm's dedup key from 30 s to 15 minutes (`:141`); an error leaves the 30-second key, which
is BF-125's retry. With several keys, one key's success extends the key even when another key's
call failed, so the failed key is not retried for 15 minutes.

**Reproduced in-process 2026-09-25** with two keys against a local stand-in for IFTTT: an alarm made
6 requests in interleaved order, and the final callback ran 4 times. The reproduction script was
in a scratch directory and was not retained. The missed retry follows from the 15-minute key and
is read, not run. `lib/plugins/maker.js` and `pushnotify.js` are identical on `v15.0.8` `92d08342`
and `dev` `e3adc91d` (`git diff` is empty); `bf/maker-level-names` does not touch `maker.js`.

**Severity.** Low: an alarm to one of several Maker keys can be missed for up to 15 minutes when
that key's call fails, and the three events arrive out of order. Bounded to sites with more than
one Maker key.

**Decision 2026-09-26 (maintainer):** after 15.0.9.

**Fix shape (not measured):** have `makeKeyRequests` call back once, after every key's request has
answered (for example with `async.each`), and report an error if any key failed, so the retry
covers the failed key. A `tests/maker.test.js` case with two keys and a failing stub.

### BF-138 · `/pebble` in the other units computes the Bolus Wizard Preview against the wrong settings

**Mechanism.** `/pebble` adds the Bolus Wizard Preview fields `bwp` and `bwpo` from the `bwp`
plugin run on its own sandbox (`lib/server/pebble.js:95-99`). `prepareSandbox` (`:143-154`)
switches that sandbox to mmol/L when the request asks for it (`:146-148`), but the profile's
sensitivity and targets stay in the site's units. On an mg/dL site asked for `?units=mmol`, the
plugin compares an mmol/L reading with mg/dL settings.

**Reproduced in-process 2026-09-25** on `v15.0.8` and `dev`, through the pebble middleware and
handler as for BF-128: sensitivity 70, targets 90-126, 1 U given 30 minutes before. The site's own
values are `bwp` −0.96 and `bwpo` 23; `?units=mmol` gives `bwp` −2.17 and `bwpo` −61.8. On an
mmol/L site asked for mg/dL, `bwpo` stays in mmol/L next to an mg/dL reading, unlabelled.
`bf/pebble-delta-units` `aa224c69` does not change either.

**Corrected 2026-09-26 on a booted server.** The usual wrong output on an mg/dL site asked for
mmol is `bwp` −0.96, right by coincidence, with `bwpo` 23.2: an mg/dL number in an mmol response.
That is because of BF-139. The −2.17 / −61.8 pair appears only in-process on fresh data, or between
a data load and its evaluation. On an mmol/L site asked for mg/dL, `bwpo` stayed in mmol/L (1.3)
next to an mg/dL reading.

**Severity.** Safety, dosing-relevant display on a watch face: the bolus wizard preview is wrong in
size and sign, and nothing marks it. Bounded to clients that ask `/pebble` for the units the site
does not use. This is not medical advice; a dosing decision belongs to the app that doses.

**Decision 2026-09-26 (maintainer):** fixed in 15.0.9, with BF-128 and BF-139 on
`bf/pebble-delta-units`, one PR. The fix must not switch the sandbox units the other way:
BF-128's detail shows that gives a wrong `bwp` too.

**Fixed 2026-09-26** on local branch `bf/pebble-delta-units` `b06eb014` (on `df149642`, BF-139,
and `aa224c69`, BF-128; not pushed; Node 22.23.2, MongoDB 7.0.43). `iob`, `cob` and `bwp` are
computed in a sandbox in the site's units; `bwpo` is converted to the requested units; `bwp`, an
insulin amount, is not converted. Across 18 combinations of site units and request, only `bwp` and
`bwpo` change, in the two mismatched combinations, plus BF-128's delta. One BF-128 test
expectation changed by design: `bwpo` now moves with `?units=mgdl`. Suite on `b06eb014` 3216/0/3,
25 new tests (4 for BF-139, 21 for BF-138).

### BF-139 · a `/pebble` request can leave glucose values scaled in its units for other readers

Described by mechanism only: it is live on 15.0.8.

**Mechanism (read).** The sandbox a `/pebble` request builds works on the server's data through
`ddata.clone()`, which is a shallow copy (`lib/data/ddata.js:109-111`): the reading objects are
the server's own. `sbx.scaleEntry` (`lib/sandbox.js:258-268`), which `bgnow` calls for each reading
(`lib/plugins/bgnow.js:85`), writes `entry.scaled` onto the reading when it is not already set. A
request whose units differ from the site's (`prepareSandbox`, `lib/server/pebble.js:143-154`) can
therefore leave the readings carrying a `scaled` value in its units, and until the next data load
replaces those objects, other readers of the same objects can see values in the wrong units. The
code is the same on `v15.0.8` `92d08342` and `dev` `e3adc91d`.

**Reach (2026-09-26).** It reaches the server's own alarm evaluation (reproduced on a booted
server 2026-09-26). Severity raised to **safety** (alarm integrity). Detail withheld pending the
maintainer's disclosure decision.

**Decision 2026-09-26 (maintainer):** fixed in 15.0.9, with BF-128 and BF-138 on
`bf/pebble-delta-units`, one PR.

**Fixed 2026-09-26** on local branch `bf/pebble-delta-units` `df149642` (on `aa224c69`; not
pushed): the `/pebble` sandbox gets its own copies of the readings, without a stored scaled
value, so a `/pebble` request no longer writes onto the server's readings.
