...
Entries name the refs they were measured on. Status and the counts below are as of 2026-09-25,
...
|---|---|---|
| How many §1 defects are there, and how many reach an operator on 15.0.8? | **95** §1 defects (every §1 entry except BF-12, invalid, and BF-41, closed): **27 open, 59 merged, 1 partly merged (BF-07), 8 fixed or partly fixed on unmerged local branches** (BF-52, BF-106, BF-123, BF-128, BF-129 and BF-135 fixed; BF-122 and BF-125 partly fixed). **93** reach an operator on 15.0.8 — BF-80 and BF-106 exist only on `dev` (below) | `node tools/queue/gates/register-exposure-legend.js` (it counts BF-07 under open) |
| How much work is outstanding? | **53 ids not repaired**, plus the halves left on BF-122 (v1 deletes) and BF-125 (the resend) | `make queue-coverage` (counts `partly merged` as not fixed and `partly fixed` as fixed) |
...
On 2026-09-25 the highest is BF-135; BF-136 to BF-139 are held for filings that await the maintainer, so the next free id is BF-140. BF-82, BF-83 and BF-84 are reserved for three connection-pooler
...
would not cover every affected viewer. The choice among the options awaits the maintainer.
...
the socket. The entry stays open until the maintainer chooses among three options.
...
- **The delete design awaits the maintainer.** Options: (a) keep hard delete; (b) soft delete
  everywhere; (c) soft delete for single-record deletes; (d) hard delete plus a minimal tombstone.
...
AndroidAPS `37e3c4532a`), and, as measured above, it never applies the shift anyway. Whether to
leave it as it is awaits the maintainer.
...
**Half 2 left unchanged by design, awaiting the maintainer.** The 2015 commits `f805633f` and
`ea745065` say a failed send is retried, which is what the 30-second key does. The branch's tests
now pin that behaviour (a failing stub is retried), so a change to it is a visible decision.
...
**Candidates awaiting the maintainer (not filed).** Two further observations from the fix, not
given ids until the maintainer decides:

- On an mg/dL site asked for `?units=mmol`, `/pebble` computes the Bolus Wizard Preview in an mmol
  sandbox against the mg/dL profile (`bwp` `-2.17` where the site's own value is `-0.96`). The
  branch does not change this.
- A shared-state mechanism: the values one `/pebble` request scales can persist on data shared
  with later requests. Described as a mechanism only.
