...
|---|---|---|
| How many §1 defects are there, and how many reach an operator on 15.0.8? | **94** §1 defects (every §1 entry except BF-12, invalid, and BF-41, closed): **37 open, 50 merged, 1 partly merged (BF-07), 6 fixed on an unmerged branch** (BF-99 to BF-102, whose PR #8758 merged 2026-09-25 as 4d9ecc3b, pending their entries' update; BF-52 on a local branch; BF-134 on bf/loop-apns-shutdown). **92** reach an operator on 15.0.8 — BF-80 and BF-106 exist only on `dev` (below) | `node tools/queue/gates/register-exposure-legend.js` (it counts BF-07 under open) |
| How much work is outstanding? | **68 ids not repaired** | `make queue-coverage` (counts `partly merged` as not fixed) |
