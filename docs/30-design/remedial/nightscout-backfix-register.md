...
| question | answer, 2026-09-25 (`origin/dev` `fbaa4a2a`) | reproduce with |
|---|---|---|
| How many §1 defects are there, and how many reach an operator on 15.0.8? | **94** §1 defects (every §1 entry except BF-12, invalid, and BF-41, closed): **33 open, 58 merged, 1 partly merged (BF-07), 2 fixed on an unmerged branch** (BF-52 on a local branch; BF-134 on bf/loop-apns-shutdown). **92** reach an operator on 15.0.8 — BF-80 and BF-106 exist only on `dev` (below) | `node tools/queue/gates/register-exposure-legend.js` (it counts BF-07 under open) |
| How much work is outstanding? | **59 ids not repaired** | `make queue-coverage` (counts `partly merged` as not fixed) |
...
On 2026-09-25 the highest is BF-134. BF-82, BF-83 and BF-84 are reserved for three connection-pooler
