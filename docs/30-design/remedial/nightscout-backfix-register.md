...
|---|---|---|
| How many §1 defects are there, and how many reach an operator on 15.0.8? | **94** §1 defects (every §1 entry except BF-12, invalid, and BF-41, closed): **33 open, 59 merged, 1 partly merged (BF-07), 1 fixed on an unmerged branch** (BF-52 on a local branch). **92** reach an operator on 15.0.8 — BF-80 and BF-106 exist only on `dev` (below) | `node tools/queue/gates/register-exposure-legend.js` (it counts BF-07 under open) |
