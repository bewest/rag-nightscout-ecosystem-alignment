...
|---|---|---|
| How many §1 defects are there, and how many reach an operator on 15.0.8? | **99** §1 defects (every §1 entry except BF-12, invalid, and BF-41, closed): **31 open, 59 merged, 1 partly merged (BF-07), 8 fixed on unmerged local branches** (BF-52, BF-106, BF-122, BF-123, BF-125, BF-128, BF-129 and BF-135). Of the open ones, BF-80, BF-121, BF-136, BF-138 and BF-139 have a fix in progress for 15.0.9 with no commit recorded yet. **97** reach an operator on 15.0.8 — BF-80 and BF-106 exist only on `dev` (below) | `node tools/queue/gates/register-exposure-legend.js` (it counts BF-07 under open) |
| How much work is outstanding? | **57 ids not repaired**. The halves left on BF-122 (v1 deletes stay hard deletes) and BF-125 (a failed call is retried) were decided as intended on 2026-09-26 | `make queue-coverage` (counts `partly merged` as not fixed and `partly fixed` as fixed) |
...
| **BF-136** | **An AndroidAPS treatment that API v3 matches to a record written through API v1 is refused, and never reaches Nightscout.** A v3 POST with no `identifier` that finds an existing record with the same `created_at` and `eventType` becomes an update (deduplication), and the update validator treats `app` and `device` as fields a client may not change. A v1-born record has no `app`; the incoming AndroidAPS record has one, so the write is answered 400 "Field app cannot be modified by the client". AndroidAPS logs a 400 and moves its upload on past the record (`NSClientV3Plugin.kt:1019`, read), so it is never retried. A v3 PUT to any v1-born record without `app` is refused the same way: 400 "Bad or missing app field" without `app`, "Field app cannot be modified" with it; the same for `device` | `lib/api3/generic/update/validate.js:21-43` (immutable fields `:21-22`, check `:39-42`); the dedup that turns the POST into an update, `lib/api3/generic/create/operation.js:43`, `:52`, with the treatments fallback `created_at` + `eventType` (`lib/api3/generic/setup.js:95`); identical on `v15.0.8` `92d08342` and `dev` `e3adc91d` | **safety** (dosing-relevant data loss in Nightscout): the AndroidAPS entry is missing from Nightscout's COB, IOB, chart, reports and what followers see, with no error on the site. The phone's own dosing is unaffected: AndroidAPS keeps its own record. Bounded to an AndroidAPS upload that lands on the `created_at` and `eventType` of a record written through v1 (for example the same meal entered in the careportal and on the phone at the same time), and to v3 edits of v1-born records | yes | **open** — **Decision 2026-09-26 (maintainer):** fixed in 15.0.9. **Fix in progress** on local branch `bf/api3-app-field-v1-records`, no commit recorded yet. **Reproduced** 2026-09-25 on `v15.0.8` `92d08342` and `dev` `e3adc91d`: 20 g carbs as a v1 `Meal Bolus`, then the same time as a v3 `Meal Bolus` from app `AAPS` with no identifier: the v3 write is answered 400 and the stored record is the v1 one. The v3 PUT refusal was measured on both builds. The AndroidAPS side is read |
...
| **BF-138** | **`/pebble` asked for the units the site does not use computes the Bolus Wizard Preview against settings in the other units.** On an mg/dL site asked for `?units=mmol`, `prepareSandbox` switches the sandbox to mmol/L while the profile's sensitivity and targets stay in mg/dL, so `bwp` and `bwpo` compare an mmol/L reading with mg/dL settings: `bwp` −2.17 and `bwpo` −61.8 where the site's own values are −0.96 and 23 (sensitivity 70, targets 90-126, 1 U given 30 min before). On an mmol/L site asked for mg/dL, `bwpo` stays in mmol/L next to an mg/dL reading, unlabelled | `lib/server/pebble.js` `prepareSandbox` `:143-154` (mmol switch `:146-148`), BWP fields `:95-99`; `lib/plugins/boluswizardpreview.js`; the same on `v15.0.8` `92d08342` and `dev` `e3adc91d` | **safety** (dosing-relevant display on a watch face): the bolus wizard preview a watch face shows is wrong in size and sign, with nothing marking it. Bounded to clients that ask `/pebble` for the units the site does not use | yes | **open** — found 2026-09-25 while fixing BF-128. **Decision 2026-09-26 (maintainer):** fixed in 15.0.9 with BF-128, on `bf/pebble-delta-units`, one PR. **Fix in progress**, no commit recorded yet. **Reproduced** in-process 2026-09-25 on `v15.0.8` and `dev` (the figures in this row); `bf/pebble-delta-units` `aa224c69` does not change it |
| **BF-139** | **A `/pebble` request can change the units of glucose values other requests read.** The sandbox a `/pebble` request builds shares the server's reading objects: `ddata.clone()` is a shallow copy, and `sbx.scaleEntry` (called by `bgnow` for each reading) writes `entry.scaled` onto the reading when it is not already set. A request in units other than the site's can therefore leave readings scaled in its units, and until the next data load other readers of the same objects can see values in the wrong units. Whether the server's alarm evaluation can see it is being measured. Described by mechanism only | `lib/data/ddata.js` `clone` `:109-111` (shallow); `lib/sandbox.js` `scaleEntry` `:258-268`; `lib/plugins/bgnow.js:85`; `lib/server/pebble.js` `prepareSandbox` `:143-154`; the same on `v15.0.8` `92d08342` and `dev` `e3adc91d` (read) | **correctness**, possibly more: glucose values in the wrong units on other displays until the next data load, reachable with `api:pebble` read permission alone. Whether an alarm can be affected is not yet established | yes | **open** — noticed 2026-09-25 while fixing BF-128. **Decision 2026-09-26 (maintainer):** fixed in 15.0.9 with BF-128, on `bf/pebble-delta-units`, one PR. **Fix in progress**, no commit recorded yet. **Read-derived**: the mechanism is from source; its reach to other readers and to the alarm evaluation is being measured |
...
- **Fix in progress for 15.0.9** on local branch `bf/alarm-anonymous-no-delay` (worktree
  `externals/work/crm-r2-fix-80`), no commit recorded yet. It narrows a brute-force control, so
...
**Fix in progress for 15.0.9** on local branch `bf/same-time-treatments` (worktree
`externals/work/crm-r2-fix-121`), no commit recorded yet.
...
- The probe's v1 DELETE arm now measures the decision, not a pending defect: the record is
  ...
  "a v1 DELETE still removes the record, and history does not report it" pins it; its name and
  comment on `d45987f7` still say "decision pending".
...
Fix in progress, no commit recorded yet.
...
`identifier` during deduplication. A record written through v1 has no `app`. An AndroidAPS record
has one, so the comparison at `:39` fails and the POST is answered 400
"Field app cannot be modified by the client". The same holds for `device` when the two differ.

A v3 PUT to a v1-born record without `app` is refused by the same list: a body without `app` is
400 "Bad or missing app field", and a body with it is 400 "Field app cannot be modified". So a
v1-born record cannot be edited through v3 at all.

**What AndroidAPS does with the 400** (AndroidAPS `7e1d537d49`, read): `NSClientV3Plugin.kt:1019`
logs "◄ FAIL" and falls through, so the upload moves on past the record and does not retry it
(a 404 is retried; a 400 is not). The entry stays on the phone and never reaches Nightscout.
...
**Decision 2026-09-26 (maintainer):** fixed in 15.0.9. **Fix in progress** on local branch
`bf/api3-app-field-v1-records` (worktree `externals/work/crm-r2-fix-136`), no commit recorded yet.
...
`bf/pebble-delta-units`, one PR. **Fix in progress**, no commit recorded yet. The fix must not
switch the sandbox units the other way: BF-128's detail shows that gives a wrong `bwp` too.

### BF-139 · a `/pebble` request can leave glucose values scaled in its units for other readers

Described by mechanism only: it is live on 15.0.8 and needs only `api:pebble` read permission.
...
**Not yet established:** which readers use `scaled` from these objects, and whether the server's
alarm evaluation can see it. That is being measured. The severity stays **correctness** until it
is.

**Decision 2026-09-26 (maintainer):** fixed in 15.0.9, with BF-128 and BF-138 on
`bf/pebble-delta-units`, one PR. **Fix in progress**, no commit recorded yet.
