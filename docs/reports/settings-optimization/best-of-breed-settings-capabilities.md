...
| **Demand-phase ISF** | ✅ **Productionized** | EXP-2651, 2663–2667, 2680–2683 | `compute_demand_isf()` with 6h Nyquist-correct isolation, tiered fallback, carb exclusion. `advise_isf()` targets demand ISF with conservative 25% step. Demand ISF confirmed circadian-flat but IS dose-dependent at large N (r=−0.418, EXP-2680) — a ratio artifact since BG drop ≈ constant regardless of dose (EXP-2681). Useful as per-patient aggregate for relative comparisons. |
