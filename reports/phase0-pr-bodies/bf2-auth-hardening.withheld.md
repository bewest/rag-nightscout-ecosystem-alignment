...
- Against #8605 it conflicts in adjacent lines only. Three deliberate differences: with `TRUST_PROXY` unset, the client address is resolved exactly as `dev` does, not as #8605's default does; `TRUST_PROXY` here also accepts hop counts and `true`, which #8605 refuses; and the v1 API app does not set its own `trust proxy`, because it already inherits the server's (#8605 sets it on both).
