...
**DRAFT. Not pushed, not opened.** Branch `bf/count-client-compat` on `origin/dev` `ddd9b600`, one
commit, `b4ead206`. No `CHANGELOG.md` edit. The version stays 15.0.9. This implements the
...
- **Both rewrites set `Deprecation: true` and `Warning: 299 - "…"`** and `console.warn` once per process per shape.
