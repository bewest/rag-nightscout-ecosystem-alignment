...
def train_bridge(model, train_x, val_x, save_path, label, device,
                 ...
                 future_steps=None, augment_std=0.0, loss_fn=None):
    ...
    loss_fn: if provided, use instead of MSELoss. Signature: loss_fn(pred, target).
    """
    model.to(device)
...
# ─── EXP-463: Triple Stack — Transfer + Fidelity + 2nd-Order PK ───

def run_exp463(args):
    """EXP-463: Stack all three proven levers at w96.

    Combines:
    1. Window transfer (w48→w96): −0.93 overall (EXP-462)
    2. Fidelity filtering: −0.55 at w96 (EXP-461)
    3. 2nd-order PK derivatives: −0.66 h240 (EXP-459)

    Key question: do these improvements compound or cannibalize each other?
    If they're orthogonal, expect −1.5 to −2.0 overall.
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    ft_ep = min(cfg['epochs_ft'] * 2, 30)
    print(f"\n{'='*60}")
    print(f"EXP-463: Triple Stack (Transfer + Fidelity + 2nd PK)")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}, ft_ep={ft_ep}")
    print(f"{'='*60}")

    arch = {'d_model': 64, 'nhead': 4, 'num_layers': 4}

    # ── Phase 1: Train w48 base on standard PK ──
    data48 = load_bridge_data(
        args.patients_dir, window_size=48,
        max_patients=cfg['max_patients'], load_isf=True)
    has_isf = 'isf_val' in data48
    train48, val48 = prepare_pk_future(data48, use_isf=has_isf, drop_time=True)
    n_ch_48 = train48.shape[-1]

    print(f"\n  Phase 1: Train w48 base ({n_ch_48}ch)")
    w48_states = {}
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch_48, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp463_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'463-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']

    # ── Phase 2: Prepare w96 data with 2nd-order PK ──
    data96 = load_bridge_data(
        args.patients_dir, window_size=96,
        max_patients=cfg['max_patients'], load_isf=True)
    isf_v96 = data96.get('isf_val')
    ...
    train96_2nd, val96_2nd = _prepare_pk_second_order(data96, history_steps=48, use_isf=has_isf)
    n_ch_96 = train96_2nd.shape[-1]
    print(f"  2nd-order PK: {n_ch_96}ch")

    # Also standard PK for control
    train96_std, val96_std = prepare_pk_future(data96, use_isf=has_isf, drop_time=True)

    # ── Phase 3: Compute fidelity and filter ──
    fidelity = {}
    for pinfo in data96['per_patient']:
        pid = pinfo['name']
        ti, te = pinfo['train_idx']
        p_train = train96_2nd[ti:te]
        pk_var = float(np.mean(np.var(p_train[:, :, 1:6].numpy(), axis=1)))
        pk_active = float(np.mean(np.any(np.abs(p_train[:, :, 3:6].numpy()) > 0.01, axis=1)))
        gluc, iob = p_train[:, :, 0].numpy(), p_train[:, :, 1].numpy()
        corrs = []
    ...
    keep_mask = np.zeros(len(train96_2nd), dtype=bool)
    for pinfo in data96['per_patient']:
        if pinfo['name'] in kept:
            ti, te = pinfo['train_idx']
            keep_mask[ti:te] = True
    train_filt = train96_2nd[keep_mask]
    print(f"  Filtered train: {len(train_filt)} (from {len(train96_2nd)})")

    # ── Phase 4: Four variants ──
    # 1. scratch_std: w96 from scratch, standard PK (control)
    # 2. transfer_only: w48→w96 transfer, standard PK
    # 3. combined_no_transfer: fidelity + 2nd PK, no transfer (= EXP-461 repro)
    # 4. triple_stack: transfer + fidelity + 2nd PK

    # Note: transfer into 2nd-order model requires matching input_dim.
    # w48 has n_ch_48=7; w96 2nd-order has n_ch_96=13.
    # Transfer strategy: only copy compatible layers (transformer weights, not projections)

    variants = OrderedDict([
        ('scratch_std', {
            'transfer': False, 'train': train96_std, 'val': val96_std,
            'n_ch': train96_std.shape[-1], 'train_all': train96_std,
            'label': 'scratch, std PK'}),
        ('transfer_std', {
            'transfer': True, 'train': train96_std, 'val': val96_std,
            'n_ch': train96_std.shape[-1], 'train_all': train96_std,
            'label': 'w48 transfer, std PK'}),
        ('combined_no_xfer', {
            'transfer': False, 'train': train_filt, 'val': val96_2nd,
            'n_ch': n_ch_96, 'train_all': train96_2nd,
            'label': 'fidelity + 2nd PK, no transfer'}),
        ('triple_stack', {
            'transfer': True, 'train': train_filt, 'val': val96_2nd,
            'n_ch': n_ch_96, 'train_all': train96_2nd,
            'label': 'transfer + fidelity + 2nd PK'}),
    ])
    ...
    for vname, vcfg in variants.items():
        ...
        for seed in cfg['seeds']:
            ...
            if vcfg['transfer']:
                # Transfer compatible layers from w48
                w48_sd = w48_states[seed]
                w96_sd = model.state_dict()
                transferred = 0
                for key in w48_sd:
                    if 'pos_encoder' in key:
                        continue
                    if key in w96_sd and w48_sd[key].shape == w96_sd[key].shape:
                        w96_sd[key] = w48_sd[key]
                        transferred += 1
                model.load_state_dict(w96_sd)
                print(f"  Transferred {transferred} params from w48")

            sp = os.path.join(cfg['output_dir'], f'exp463_{vname}_s{seed}.pth')
            print(f"\n  Base s{seed}:")
            train_bridge(model, tx, vx, sp, f'463-{vname}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                         future_steps=future_steps)

        # Per-patient FT using ALL patients (rescue dropped ones via FT)
        per_patient = {}
        for pinfo in data96['per_patient']:
            pid = pinfo['name']
            ti, te = pinfo['train_idx']
            vi, ve = pinfo['val_idx']
            p_train = vcfg['train_all'][ti:te]
            p_val = vx[vi:ve]
            p_isf = isf_v96[vi:ve] if isf_v96 is not None else None
            ...
            for seed in cfg['seeds']:
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                m.load_state_dict(ckpt['model_state'])
                fp = os.path.join(cfg['output_dir'], f'exp463_{vname}_{pid}_s{seed}.pth')
                train_bridge(m, p_train, p_val, fp, f'463-{vname}-{pid}-s{seed}',
                             device, pk_mode=True,
                             epochs=ft_ep, patience=10, lr_patience=5,
                             lr=1e-4, future_steps=future_steps)
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-463 Summary: Triple Stack")
    print(f"{'='*60}")
    for vname, vdata in result.items():
        avg = vdata['average']
        print(f"  {vname:<22} overall={avg.get('overall_mae','?')}, "
              f"h60={avg.get('h60','?')}, h120={avg.get('h120','?')}, "
              f"h240={avg.get('h240','?')}")

    base = result.get('scratch_std', {}).get('average', {})
    for vname in ['transfer_std', 'combined_no_xfer', 'triple_stack']:
        comp = result.get(vname, {}).get('average', {})
        if base and comp:
            print(f"\n  Delta ({vname} - scratch_std):")
            for k in sorted(base.keys()):
                if k in comp:
                    d = round(comp[k] - base[k], 2)
                    print(f"    {k}: {d:+.2f}")

    _save_results(result, 'exp463_triple_stack', cfg)
    return result


# ─── EXP-464: Curriculum Transfer (w24 → w48 → w96) ───

def run_exp464(args):
    """EXP-464: Three-stage curriculum transfer learning.

    Hypothesis: Gradual window expansion (w24→w48→w96) builds hierarchical
    representations — short patterns first, then extended dynamics.
    Like training a child: learn letters → words → sentences.

    If single-hop (w48→w96) gives −0.93, does two-hop give more?
    Expected: additive benefit at h180+, where each stage adds context.
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-464: Curriculum Transfer (w24 → w48 → w96)")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")

    arch = {'d_model': 64, 'nhead': 4, 'num_layers': 4}

    # ── Stage 1: Train w24 (short-horizon specialist) ──
    data24 = load_bridge_data(
        args.patients_dir, window_size=24,
        max_patients=cfg['max_patients'], load_isf=True)
    has_isf = 'isf_val' in data24
    train24, val24 = prepare_pk_future(data24, use_isf=has_isf, drop_time=True)
    n_ch = train24.shape[-1]

    print(f"\n  Stage 1: Train w24 ({n_ch}ch, {len(train24)} windows)")
    w24_states = {}
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m24 = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp464_w24_s{seed}.pth')
        train_bridge(m24, train24, val24, sp, f'464-w24-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w24_states[seed] = ckpt['model_state']

    # ── Stage 2: Transfer w24→w48 ──
    data48 = load_bridge_data(
        args.patients_dir, window_size=48,
        max_patients=cfg['max_patients'], load_isf=True)
    train48, val48 = prepare_pk_future(data48, use_isf=has_isf, drop_time=True)

    print(f"\n  Stage 2: Transfer w24 → w48 ({n_ch}ch, {len(train48)} windows)")
    w48_states = {}
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
        # Transfer from w24
        w24_sd = w24_states[seed]
        w48_sd = m48.state_dict()
        transferred = 0
        for key in w24_sd:
            if 'pos_encoder' in key:
                continue
            if key in w48_sd and w24_sd[key].shape == w48_sd[key].shape:
                w48_sd[key] = w24_sd[key]
                transferred += 1
        m48.load_state_dict(w48_sd)
        print(f"  Transferred {transferred} params w24→w48")

        sp = os.path.join(cfg['output_dir'], f'exp464_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'464-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']

    # ── Stage 3: Transfer w48→w96 (single-hop and two-hop) ──
    data96 = load_bridge_data(
        args.patients_dir, window_size=96,
        max_patients=cfg['max_patients'], load_isf=True)
    isf_v96 = data96.get('isf_val')
    ...
    variants = OrderedDict([
        ('scratch', {'transfer_from': None, 'label': 'w96 from scratch'}),
        ('single_hop', {'transfer_from': 'w48_direct', 'label': 'w48(scratch)→w96'}),
        ('two_hop', {'transfer_from': 'w48_curriculum', 'label': 'w24→w48→w96'}),
    ])

    # Also train a direct w48 (no w24 init) for single_hop comparison
    print(f"\n  Train direct w48 (no curriculum) for single-hop control")
    w48_direct_states = {}
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48d = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp464_w48direct_s{seed}.pth')
        train_bridge(m48d, train48, val48, sp, f'464-w48d-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_direct_states[seed] = ckpt['model_state']

    result = {}
    for vname, vcfg in variants.items():
        print(f"\n{'─'*40}")
        print(f"  {vname}: {vcfg['label']} ({n_ch}ch)")
        print(f"{'─'*40}")
        ...
        for seed in cfg['seeds']:
            ...
            source_states = None
            if vcfg['transfer_from'] == 'w48_direct':
                source_states = w48_direct_states
            elif vcfg['transfer_from'] == 'w48_curriculum':
                source_states = w48_states  # w24→w48 curriculum

            if source_states is not None:
                src_sd = source_states[seed]
                tgt_sd = model.state_dict()
                transferred = 0
                for key in src_sd:
                    if 'pos_encoder' in key:
                        continue
                    if key in tgt_sd and src_sd[key].shape == tgt_sd[key].shape:
                        tgt_sd[key] = src_sd[key]
                        transferred += 1
                model.load_state_dict(tgt_sd)
                print(f"  Transferred {transferred} params")

            sp = os.path.join(cfg['output_dir'], f'exp464_{vname}_s{seed}.pth')
            print(f"\n  Base s{seed}:")
            train_bridge(model, train96, val96, sp, f'464-{vname}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                         future_steps=future_steps)
        ...
        for pinfo in data96['per_patient']:
            ...
            for seed in cfg['seeds']:
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                m.load_state_dict(ckpt['model_state'])
                fp = os.path.join(cfg['output_dir'], f'exp464_{vname}_{pid}_s{seed}.pth')
                train_bridge(m, p_train, p_val, fp, f'464-{vname}-{pid}-s{seed}',
                             device, pk_mode=True,
                             epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                             lr=1e-4, future_steps=future_steps)
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-464 Summary: Curriculum Transfer")
    print(f"{'='*60}")
    for vname, vdata in result.items():
        avg = vdata['average']
        print(f"  {vname:<18} overall={avg.get('overall_mae','?')}, "
    ...
    base = result.get('scratch', {}).get('average', {})
    for vname in ['single_hop', 'two_hop']:
        comp = result.get(vname, {}).get('average', {})
        if base and comp:
            print(f"\n  Delta ({vname} - scratch):")
            for k in sorted(base.keys()):
    ...
    _save_results(result, 'exp464_curriculum_transfer', cfg)
    return result


# ─── EXP-465: Extended Transfer w48→w144 for h360+ ───

def run_exp465(args):
    """EXP-465: Transfer w48 to w144 for ultra-long-horizon forecasting (h360-h480).

    Hypothesis: w96 reaches h360=20.68 (EXP-435). With w144 (6h history + 6h future),
    we can push h360 below 20 and reach h480 (8h = full DIA).
    Transfer from w48 provides strong base representations.

    This tests the practical limit of glucose forecasting at DIA timescale.
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-465: Extended Transfer (w48 → w144, h360+)")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp465_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'465-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']
    ...
    future_steps = 72  # 6h future for w144

    print(f"\n  Phase 2: w144 data ({train144.shape[-1]}ch, {len(train144)} windows)")
    print(f"  future_steps={future_steps} (h360 max)")

    variants = OrderedDict([
        ('w144_scratch', {'transfer': False, 'label': 'w144 from scratch'}),
        ('w144_transfer', {'transfer': True, 'label': 'w48→w144 transfer'}),
    ])
    ...
    for vname, vcfg in variants.items():
        ...
        for seed in cfg['seeds']:
            ...
            sp = os.path.join(cfg['output_dir'], f'exp465_{vname}_s{seed}.pth')
            print(f"\n  Base s{seed}:")
            train_bridge(model, train144, val144, sp, f'465-{vname}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                         future_steps=future_steps)
        ...
        for pinfo in data144['per_patient']:
            ...
            for seed in cfg['seeds']:
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                m.load_state_dict(ckpt['model_state'])
                fp = os.path.join(cfg['output_dir'], f'exp465_{vname}_{pid}_s{seed}.pth')
                train_bridge(m, p_train, p_val, fp, f'465-{vname}-{pid}-s{seed}',
                             device, pk_mode=True,
                             epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                             lr=1e-4, future_steps=future_steps)
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-465 Summary: Extended Transfer w48→w144")
    print(f"{'='*60}")
    for vname, vdata in result.items():
        avg = vdata['average']
        print(f"  {vname:<18} overall={avg.get('overall_mae','?')}, "
    ...
    scratch = result.get('w144_scratch', {}).get('average', {})
    transfer = result.get('w144_transfer', {}).get('average', {})
    if scratch and transfer:
        print(f"\n  Delta (transfer - scratch):")
        for k in sorted(scratch.keys()):
            if k in transfer:
    ...
    _save_results(result, 'exp465_extended_transfer', cfg)
    return result


# ─── EXP-466: Routing Comparison — w96 vs w144 Transfer for h180+ ───

def run_exp466(args):
    """EXP-466: Compare w96_transfer vs w144_transfer for production routing.

    Question: For h180-h360 predictions, which is the better long specialist?
    - w96_transfer: less data loss (1618 vs 1078 windows/pt), proven h240=27.64
    - w144_transfer: native h360 coverage, but fewer windows and harder task

    Also tests: does combining their predictions (simple average) help?
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-466: Routing w96 vs w144 Transfer Comparison")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")

    arch = {'d_model': 64, 'nhead': 4, 'num_layers': 4}

    # ── Train w48 base (shared for both transfers) ──
    data48 = load_bridge_data(
        args.patients_dir, window_size=48,
        max_patients=cfg['max_patients'], load_isf=True)
    has_isf = 'isf_val' in data48
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp466_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'466-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']

    def _transfer_and_train(data, window_size, future_steps, suffix):
        """Train transferred model and return per-patient reports."""
        isf_v = data.get('isf_val')
        train_x, val_x = prepare_pk_future(data, use_isf=has_isf, drop_time=True)
        ...
        for seed in cfg['seeds']:
            ...
            for key in w48_sd:
                ...
                if key in m_sd and w48_sd[key].shape == m_sd[key].shape:
                    m_sd[key] = w48_sd[key]; transferred += 1
            model.load_state_dict(m_sd)
            print(f"  Transferred {transferred} params w48→w{window_size}")

            sp = os.path.join(cfg['output_dir'], f'exp466_{suffix}_s{seed}.pth')
            train_bridge(model, train_x, val_x, sp, f'466-{suffix}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                         future_steps=future_steps)
        ...
        for pinfo in data['per_patient']:
            ...
            for seed in cfg['seeds']:
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                m.load_state_dict(ckpt['model_state'])
                fp = os.path.join(cfg['output_dir'], f'exp466_{suffix}_{pid}_s{seed}.pth')
                train_bridge(m, p_train, p_val, fp, f'466-{suffix}-{pid}-s{seed}',
                             device, pk_mode=True,
                             epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                             lr=1e-4, future_steps=future_steps)
    ...
    print(f"\n{'─'*40}\n  w96_transfer\n{'─'*40}")
    pp96 = _transfer_and_train(data96, 96, 48, 'w96')

    print(f"\n{'─'*40}\n  w144_transfer\n{'─'*40}")
    pp144 = _transfer_and_train(data144, 144, 72, 'w144')

    # ── Compare at overlapping horizons ──
    result = {}
    for label, pp in [('w96_transfer', pp96), ('w144_transfer', pp144)]:
        avg = {}
        ...
        print(f"\n  {label} avg: overall={avg.get('overall_mae','?')}, "
              f"h60={avg.get('h60','?')}, h120={avg.get('h120','?')}, "
              f"h240={avg.get('h240','?')}, h360={avg.get('h360','?')}")

    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-466 Summary: Routing Comparison")
    print(f"{'='*60}")
    for label, vdata in result.items():
        avg = vdata['average']
        keys = ['overall_mae', 'h60', 'h120', 'h180', 'h240', 'h300', 'h360']
        vals = [f"{k}={avg.get(k,'—')}" for k in keys if k in avg]
        print(f"  {label}: {', '.join(vals)}")

    _save_results(result, 'exp466_routing_comparison', cfg)
    return result


# ─── EXP-467: Transfer + Extended FT ───

def run_exp467(args):
    """EXP-467: Does extended per-patient FT compound with transfer at w96?

    Transfer gives better base initialization. Extended FT (30ep vs 15ep)
    gives more patient adaptation. Do they compound?

    Also tests: very long FT (45ep) to find diminishing returns.
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-467: Transfer + Extended FT at w96")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp467_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'467-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']
    ...
    # Train w96 base with transfer
    print(f"\n  Train w96 with transfer")
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        model = PKGroupedEncoder(input_dim=n_ch, **arch)
        w48_sd = w48_states[seed]
        ...
        sp = os.path.join(cfg['output_dir'], f'exp467_w96_s{seed}.pth')
        train_bridge(model, train96, val96, sp, f'467-w96-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                     future_steps=future_steps)

    # FT variants
    ft_epochs_list = [15, 30, 45]

    result = {}
    for ft_ep in ft_epochs_list:
        vname = f'ft{ft_ep}'
        print(f"\n{'─'*40}")
        print(f"  {vname}: transfer + {ft_ep}ep FT")
        print(f"{'─'*40}")
        ...
        for pinfo in data96['per_patient']:
            ...
            for seed in cfg['seeds']:
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                m.load_state_dict(ckpt['model_state'])
                fp = os.path.join(cfg['output_dir'], f'exp467_{vname}_{pid}_s{seed}.pth')
                train_bridge(m, p_train, p_val, fp, f'467-{vname}-{pid}-s{seed}',
                             device, pk_mode=True,
                             epochs=ft_ep, patience=10, lr_patience=5,
                             lr=1e-4, future_steps=future_steps)
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-467 Summary: Transfer + Extended FT")
    print(f"{'='*60}")
    for vname, vdata in result.items():
        avg = vdata['average']
        print(f"  {vname:<12} overall={avg.get('overall_mae','?')}, "
              f"h60={avg.get('h60','?')}, h120={avg.get('h120','?')}, "
              f"h240={avg.get('h240','?')}")

    base = result.get('ft15', {}).get('average', {})
    for vname in ['ft30', 'ft45']:
        comp = result.get(vname, {}).get('average', {})
        if base and comp:
            print(f"\n  Delta ({vname} - ft15):")
            for k in sorted(base.keys()):
    ...
    _save_results(result, 'exp467_transfer_extended_ft', cfg)
    return result


# ─── EXP-468: Asymmetric Window with Transfer ───

def run_exp468(args):
    """EXP-468: Asymmetric w96 (history=64, future=32) with w48 transfer.

    EXP-435 showed w96_asym (h=48, f=48) reaches h360=20.68.
    What if we shift the split: more history (64 steps = 5h20m) but
    keep future at 32 (h160)? The hypothesis: for h120-h180, more
    history context matters more than longer future projection.
    Transfer from w48 provides strong initial representations.
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-468: Asymmetric Window + Transfer")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp468_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'468-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']
    ...
    # Asymmetric splits: (history, future)
    variants = OrderedDict([
        ('sym_48_48', {'future_steps': 48, 'label': 'symmetric 48h+48f'}),
        ('asym_64_32', {'future_steps': 32, 'label': 'asymmetric 64h+32f'}),
    ])
    ...
    for vname, vcfg in variants.items():
        ...
        for seed in cfg['seeds']:
            ...
            sp = os.path.join(cfg['output_dir'], f'exp468_{vname}_s{seed}.pth')
            print(f"\n  Base s{seed}:")
            train_bridge(model, train96, val96, sp, f'468-{vname}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                         future_steps=fs)
        ...
        for pinfo in data96['per_patient']:
            ...
            for seed in cfg['seeds']:
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                m.load_state_dict(ckpt['model_state'])
                fp = os.path.join(cfg['output_dir'], f'exp468_{vname}_{pid}_s{seed}.pth')
                train_bridge(m, p_train, p_val, fp, f'468-{vname}-{pid}-s{seed}',
                             device, pk_mode=True,
                             epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                             lr=1e-4, future_steps=fs)
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-468 Summary: Asymmetric Window + Transfer")
    print(f"{'='*60}")
    for vname, vdata in result.items():
        avg = vdata['average']
        keys = ['overall_mae', 'h30', 'h60', 'h90', 'h120', 'h150', 'h160', 'h180', 'h240']
        vals = [f"{k}={avg.get(k,'—')}" for k in keys if k in avg]
        print(f"  {vname}: {', '.join(vals)}")

    sym = result.get('sym_48_48', {}).get('average', {})
    asym = result.get('asym_64_32', {}).get('average', {})
    if sym and asym:
        print(f"\n  Delta (asym - sym):")
        all_keys = sorted(set(list(sym.keys()) + list(asym.keys())))
        for k in all_keys:
            if k in sym and k in asym:
                d = round(asym[k] - sym[k], 2)
                print(f"    {k}: {d:+.2f}")

    _save_results(result, 'exp468_asymmetric_transfer', cfg)
    return result
...
# ─── EXP-471: Horizon-Focused Loss with Asymmetric Window ───

def run_exp471(args):
    """EXP-471: Combine h60-focused loss with asymmetric window + transfer.

    Related research (EXP-424) showed h60_focus loss (3× weight on h60)
    improves BOTH h30 (−0.42) and h60 (−0.83). Our asymmetric windows
    (EXP-468/469) also dramatically help h60. Do they compound?

    Tests on asym w96 (64h+32f) — the proven sweet spot for h5-h120.
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-471: Horizon-Focused Loss + Asymmetric Window + Transfer")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp471_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'471-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']
    ...
    isf_v96 = data96.get('isf_val')
    train96, val96 = prepare_pk_future(data96, use_isf=has_isf, drop_time=True)
    future_steps = 32  # asymmetric: 64h+32f

    # Custom horizon-weighted loss
    def h60_weighted_loss(pred, target, future_steps_n):
        """MSE with 3× weight on h60 position (step 12 in future)."""
        mse = (pred - target) ** 2
        weights = torch.ones_like(mse)
        # h60 = step 12 in future (60min / 5min = 12)
        h60_idx = min(11, future_steps_n - 1)  # 0-indexed
        weights[:, h60_idx, :] = 3.0
        return (mse * weights).mean()

    variants = OrderedDict([
        ('uniform_loss', {'weighted': False, 'label': 'asym 64+32, uniform MSE'}),
        ('h60_focus', {'weighted': True, 'label': 'asym 64+32, h60 3× weight'}),
    ])
    ...
    for vname, vcfg in variants.items():
        ...
        for seed in cfg['seeds']:
            ...
            sp = os.path.join(cfg['output_dir'], f'exp471_{vname}_s{seed}.pth')
            print(f"\n  Base s{seed}:")

            if vcfg['weighted']:
                # Custom training with weighted loss
                train_bridge(model, train96, val96, sp, f'471-{vname}-s{seed}',
                             device, pk_mode=True,
                             epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                             future_steps=future_steps,
                             loss_fn=lambda p, t: h60_weighted_loss(p, t, future_steps))
            else:
                train_bridge(model, train96, val96, sp, f'471-{vname}-s{seed}',
                             device, pk_mode=True,
                             epochs=cfg['epochs_base'], patience=20, lr_patience=7,
        ...
        for pinfo in data96['per_patient']:
            ...
            for seed in cfg['seeds']:
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                m.load_state_dict(ckpt['model_state'])
                fp = os.path.join(cfg['output_dir'], f'exp471_{vname}_{pid}_s{seed}.pth')
                train_bridge(m, p_train, p_val, fp, f'471-{vname}-{pid}-s{seed}',
                             device, pk_mode=True,
                             epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                             lr=1e-4, future_steps=future_steps)
        ...
        result[vname] = {'per_patient': per_patient, 'average': avg}
        print(f"\n  {vname} avg: overall={avg.get('overall_mae','?')}, "
              f"h30={avg.get('h30','?')}, h60={avg.get('h60','?')}, "
              f"h120={avg.get('h120','?')}")

    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-471 Summary: Horizon-Focused Loss + Asymmetric")
    print(f"{'='*60}")
    for vname, vdata in result.items():
        avg = vdata['average']
    ...
    uni = result.get('uniform_loss', {}).get('average', {})
    h60f = result.get('h60_focus', {}).get('average', {})
    if uni and h60f:
        print(f"\n  Delta (h60_focus - uniform):")
        for k in sorted(uni.keys()):
            if k in h60f:
                d = round(h60f[k] - uni[k], 2)
                print(f"    {k}: {d:+.2f}")

    _save_results(result, 'exp471_h60_focus_asym', cfg)
    return result


# ─── EXP-472: Autoregressive Rollout from Short Specialist ───

def run_exp472(args):
    """EXP-472: Can rolling forward a short specialist beat direct long prediction?

    Related research (EXP-439) showed AR rollout competitive at h240/h360.
    Our asym_64_32 gives h60=18.49 and h120=23.31.
    What if we roll it forward 3× to reach h360?

    Rollout: predict h160, shift window by 32, predict again from shifted context.
    This requires using the model's own predictions as future glucose context.
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-472: Autoregressive Rollout from Short Specialist")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")

    arch = {'d_model': 64, 'nhead': 4, 'num_layers': 4}

    # Train w48 base + transfer to asym w96 (64h+32f)
    data48 = load_bridge_data(
        args.patients_dir, window_size=48,
        max_patients=cfg['max_patients'], load_isf=True)
    has_isf = 'isf_val' in data48
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp472_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'472-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)

    # Train asym w96 (64h+32f) with transfer
    data96 = load_bridge_data(
        args.patients_dir, window_size=96,
        max_patients=cfg['max_patients'], load_isf=True)
    isf_v96 = data96.get('isf_val')
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        model = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp48 = os.path.join(cfg['output_dir'], f'exp472_w48_s{seed}.pth')
        ckpt48 = torch.load(sp48, map_location=device, weights_only=False)
        m_sd = model.state_dict()
        for key in ckpt48['model_state']:
            if 'pos_encoder' in key: continue
            if key in m_sd and ckpt48['model_state'][key].shape == m_sd[key].shape:
                m_sd[key] = ckpt48['model_state'][key]
        model.load_state_dict(m_sd)

        sp = os.path.join(cfg['output_dir'], f'exp472_asym_s{seed}.pth')
        train_bridge(model, train96, val96, sp, f'472-asym-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                     future_steps=future_steps)
    ...
    for pinfo in data96['per_patient']:
        ...
        for seed in cfg['seeds']:
            ...
            ckpt = torch.load(sp, map_location=device, weights_only=False)
            m.load_state_dict(ckpt['model_state'])
            fp = os.path.join(cfg['output_dir'], f'exp472_asym_{pid}_s{seed}.pth')
            train_bridge(m, p_train, p_val, fp, f'472-asym-{pid}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                         lr=1e-4, future_steps=future_steps)
            per_patient_models[(pid, seed)] = fp

    # Evaluate: direct prediction + autoregressive rollout
    # For AR, we need longer validation windows to evaluate at h240+
    # We'll use the w96 validation data and check which horizons we can reach
    result = {'direct': {}, 'ar_rollout': {}}
    ...
    for pinfo in data96['per_patient']:
        ...
        for seed in cfg['seeds']:
            # Load FT model
            m = PKGroupedEncoder(input_dim=n_ch, **arch)
            fp = per_patient_models[(pid, seed)]
            ckpt = torch.load(fp, map_location=device, weights_only=False)
            m.load_state_dict(ckpt['model_state'])
            m.eval().to(device)
            ...
            report = evaluate_model(m, p_val, device, pk_mode=True,
                                     isf_val=p_isf, future_steps=future_steps)
            result['direct'][pid] = report

            # AR rollout: use model predictions as new history
            # This is a simplified version — we shift the window and feed predictions
            half = 96 // 2  # = 48 for the w96 window
            ar_preds_all = []
            with torch.no_grad():
                for i in range(len(p_val)):
                    x = p_val[i:i+1].clone().to(device)
                    mask_future_pk(x, half, pk_mode=True)

                    # First pass: predict steps 0-31 of future
                    pred1 = m(x, causal=True)  # [1, 96, 1]
                    direct_future = pred1[0, half:half+future_steps, 0].cpu()

                    # For AR: shift window by future_steps, use predictions as new glucose
                    x2 = x.clone()
                    # Shift history: old future becomes new history
                    x2[0, :half-future_steps, :] = x[0, future_steps:half, :]
                    x2[0, half-future_steps:half, 0] = direct_future
                    # Zero out PK channels in shifted portion (unknown future PK)
                    x2[0, half-future_steps:half, 1:] = 0
                    mask_future_pk(x2, half, pk_mode=True)

                    pred2 = m(x2, causal=True)
                    ar_future = pred2[0, half:half+future_steps, 0].cpu()

                    # Combined: steps 0-31 direct, steps 32-63 AR
                    combined = torch.cat([direct_future, ar_future])
                    ar_preds_all.append(combined)

            # Evaluate AR at extended horizons
            ar_report = {}
            isf = p_isf[0].item() if p_isf is not None else 1.0
            scale = isf if isf > 1 else 400.0

            for step, label in [(5, 'h25'), (11, 'h55'), (23, 'h115'),
                                (31, 'h155'), (39, 'h195'), (47, 'h235'),
                                (55, 'h275'), (63, 'h315')]:
                errors = []
                for i in range(len(p_val)):
                    if step < len(ar_preds_all[i]):
                        pred_val = ar_preds_all[i][step].item() * scale
                        actual = p_val[i, half + min(step, p_val.shape[1]-half-1), 0].item() * scale
                        errors.append(abs(pred_val - actual))
                if errors:
                    ar_report[label] = round(float(np.mean(errors)), 2)

            result['ar_rollout'][pid] = ar_report

    # Average results
    for mode in ['direct', 'ar_rollout']:
        avg = {}
        for pid, rep in result[mode].items():
            for k, v in rep.items():
                if k not in avg: avg[k] = []
                avg[k].append(v)
        avg = {k: round(np.mean(v), 2) for k, v in avg.items()}
        result[f'{mode}_avg'] = avg

    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-472 Summary: Direct vs AR Rollout")
    print(f"{'='*60}")
    print(f"  Direct avg: {result.get('direct_avg', {})}")
    print(f"  AR rollout avg: {result.get('ar_rollout_avg', {})}")

    _save_results(result, 'exp472_ar_rollout', cfg)
    return result


# ─── EXP-473: Per-ISF Window Routing ───

def run_exp473(args):
    """EXP-473: Route patients to optimal window based on ISF.

    Related research (EXP-429) found:
    - Low ISF (25-40): w48 optimal
    - Medium ISF (49-77): w24-w36 optimal
    - High ISF (94): w72 optimal (slow insulin needs full DIA context)

    Hypothesis: Using ISF to route patients to their optimal asymmetric
    window will outperform a single universal window.

    Tests: Train asym w96 (64h+32f) and asym w144 (112h+32f),
    then route patients based on ISF threshold.
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-473: Per-ISF Window Routing")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp473_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'473-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']

    # Train two specialists with transfer
    window_configs = OrderedDict([
        ('w96_asym', {'window': 96, 'future_steps': 32,
                      'label': 'w96 64h+32f (low-medium ISF)'}),
        ('w144_asym', {'window': 144, 'future_steps': 32,
                       'label': 'w144 112h+32f (high ISF)'}),
    ])

    specialist_results = {}
    for sname, scfg in window_configs.items():
        ws = scfg['window']
        fs = scfg['future_steps']
        print(f"\n{'─'*40}")
        print(f"  {sname}: {scfg['label']}")
        ...
        for seed in cfg['seeds']:
            ...
            sp = os.path.join(cfg['output_dir'], f'exp473_{sname}_s{seed}.pth')
            train_bridge(model, train_x, val_x, sp, f'473-{sname}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                         future_steps=fs)
        ...
        for pinfo in data['per_patient']:
            ...
            p_val = val_x[vi:ve]
            p_isf = isf_v[vi:ve] if isf_v is not None else None
            isf_val = int(round(p_isf[0].item())) if p_isf is not None else 0
            ...
            for seed in cfg['seeds']:
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                m.load_state_dict(ckpt['model_state'])
                fp = os.path.join(cfg['output_dir'], f'exp473_{sname}_{pid}_s{seed}.pth')
                train_bridge(m, p_train, p_val, fp, f'473-{sname}-{pid}-s{seed}',
                             device, pk_mode=True,
                             epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                             lr=1e-4, future_steps=fs)
        ...
        specialist_results[sname] = per_patient

    # ISF-based routing: high ISF (>70) → w144, low ISF (≤70) → w96
    print(f"\n{'='*60}")
    print(f"EXP-473 Summary: Per-ISF Window Routing")
    print(f"{'='*60}")

    # Show per-patient results from both specialists
    patients = sorted(set(list(specialist_results['w96_asym'].keys()) +
                         list(specialist_results['w144_asym'].keys())))

    isf_threshold = 70
    routed_results = {}
    print(f"\n  ISF threshold: {isf_threshold}")
    print(f"  {'Patient':<8} {'ISF':<6} {'Route':<12} {'w96 h60':<10} {'w144 h60':<10} {'Best':<10}")

    for pid in patients:
        r96 = specialist_results['w96_asym'].get(pid, {})
        r144 = specialist_results['w144_asym'].get(pid, {})
        isf = r96.get('isf', r144.get('isf', 0))
        route = 'w144_asym' if isf > isf_threshold else 'w96_asym'
        h60_96 = r96.get('h60', '—')
        h60_144 = r144.get('h60', '—')

        routed = specialist_results[route].get(pid, {})
        routed_results[pid] = routed
        best = 'w144' if isinstance(h60_144, (int, float)) and isinstance(h60_96, (int, float)) and h60_144 < h60_96 else 'w96'
        print(f"  {pid:<8} {isf:<6} {route:<12} {h60_96!s:<10} {h60_144!s:<10} {best:<10}")

    # Compute routed average
    routed_avg = {}
    for pid, rep in routed_results.items():
        for k, v in rep.items():
            if isinstance(v, (int, float)) and k != 'isf':
                if k not in routed_avg: routed_avg[k] = []
                routed_avg[k].append(v)
    routed_avg = {k: round(np.mean(v), 2) for k, v in routed_avg.items()}

    # Universal averages
    for sname in ['w96_asym', 'w144_asym']:
        avg = {}
        for pid, rep in specialist_results[sname].items():
            for k, v in rep.items():
                if isinstance(v, (int, float)) and k != 'isf':
                    if k not in avg: avg[k] = []
                    avg[k].append(v)
        avg = {k: round(np.mean(v), 2) for k, v in avg.items()}
        print(f"\n  {sname} universal avg: overall={avg.get('overall_mae','?')}, h60={avg.get('h60','?')}")

    print(f"\n  ISF-routed avg: overall={routed_avg.get('overall_mae','?')}, h60={routed_avg.get('h60','?')}")

    result = {
        'specialist_results': {k: {pid: {kk: vv for kk, vv in v.items()}
                                    for pid, v in sv.items()}
                                for k, sv in specialist_results.items()},
        'routed_average': routed_avg,
        'isf_threshold': isf_threshold,
    }

    _save_results(result, 'exp473_isf_routing', cfg)
    return result


# ─── EXP-474: Best Combination — w144 asym + h60 focus + transfer ───

def run_exp474(args):
    """EXP-474: Combine all proven techniques on w144 asymmetric.

    EXP-469: asym_112_32 (w144) = h60 18.05 (best so far)
    EXP-471: h60_focus helped h120 (−0.46) on w96
    EXP-473: w144_asym universally beats w96_asym (17.80 vs 18.49 h60)

    Now combine: w144 asym_112_32 + h60_focus loss + w48 transfer.
    Also test h90_focus (upweight h90 position) since h120+ is where
    the remaining improvement lives.
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-474: w144 Asymmetric + Horizon Focus + Transfer")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp474_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'474-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']
    ...
    isf_v144 = data144.get('isf_val')
    train144, val144 = prepare_pk_future(data144, use_isf=has_isf, drop_time=True)
    future_steps = 32  # 112h + 32f

    # Loss functions
    def make_horizon_focus_loss(focus_step, weight=3.0):
        """Create loss that upweights a specific horizon step."""
        def loss_fn(pred, target):
            mse = (pred - target) ** 2
            w = torch.ones_like(mse)
            idx = min(focus_step, pred.shape[1] - 1)
            w[:, idx, :] = weight
            return (mse * w).mean()
        return loss_fn

    variants = OrderedDict([
        ('uniform', {'loss_fn': None, 'label': 'asym 112+32, uniform MSE'}),
        ('h60_focus', {'loss_fn': make_horizon_focus_loss(11, 3.0),
                       'label': 'asym 112+32, h60 3× weight'}),
        ('h90_focus', {'loss_fn': make_horizon_focus_loss(17, 3.0),
                       'label': 'asym 112+32, h90 3× weight'}),
        ('h120_focus', {'loss_fn': make_horizon_focus_loss(23, 3.0),
                        'label': 'asym 112+32, h120 3× weight'}),
    ])
    ...
    for vname, vcfg in variants.items():
        ...
        for seed in cfg['seeds']:
            ...
            sp = os.path.join(cfg['output_dir'], f'exp474_{vname}_s{seed}.pth')
            print(f"\n  Base s{seed}:")
            train_bridge(model, train144, val144, sp, f'474-{vname}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                         future_steps=future_steps, loss_fn=vcfg['loss_fn'])
        ...
        for pinfo in data144['per_patient']:
            ...
            for seed in cfg['seeds']:
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                m.load_state_dict(ckpt['model_state'])
                fp = os.path.join(cfg['output_dir'], f'exp474_{vname}_{pid}_s{seed}.pth')
                train_bridge(m, p_train, p_val, fp, f'474-{vname}-{pid}-s{seed}',
                             device, pk_mode=True,
                             epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                             lr=1e-4, future_steps=future_steps)
    ...
    # Summary & deltas
    print(f"\n{'='*60}")
    print(f"EXP-474 Summary: w144 Asymmetric + Horizon Focus")
    print(f"{'='*60}")
    for vname, vdata in result.items():
        avg = vdata['average']
        keys = ['overall_mae', 'h30', 'h60', 'h90', 'h120', 'h150']
    ...
    uni = result.get('uniform', {}).get('average', {})
    for vname in ['h60_focus', 'h90_focus', 'h120_focus']:
        vavg = result.get(vname, {}).get('average', {})
        if uni and vavg:
            print(f"\n  Delta ({vname} - uniform):")
            for k in sorted(uni.keys()):
                if k in vavg:
                    d = round(vavg[k] - uni[k], 2)
                    print(f"    {k}: {d:+.2f}")

    _save_results(result, 'exp474_w144_horizon_focus', cfg)
    return result


# ─── EXP-475: Extended History Sweep w192/w240 ───

def run_exp475(args):
    """EXP-475: Does even more history help? w192 (16h) and w240 (20h).

    EXP-469: asym_112_32 (w144, 9h20m history) = best so far.
    EXP-473: w144 universally beat w96.
    
    Hypothesis: More history continues to help, especially for high-ISF
    patients where insulin action spans the full DIA (~6h).
    w192 = 160h+32f = 13h20m history.
    w240 = 208h+32f = 17h20m history (covers 2× DIA).
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-475: Extended History w192/w240 + Transfer")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp475_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'475-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']

    # Window variants: w144 (control) + w192 + w240
    window_configs = OrderedDict([
        ('w144_112_32', {'window': 144, 'future_steps': 32,
                         'label': '9h20m history + 2h40m future'}),
        ('w192_160_32', {'window': 192, 'future_steps': 32,
                         'label': '13h20m history + 2h40m future'}),
        ('w240_208_32', {'window': 240, 'future_steps': 32,
                         'label': '17h20m history + 2h40m future'}),
    ])

    result = {}
    for wname, wcfg in window_configs.items():
        ws = wcfg['window']
        fs = wcfg['future_steps']
        print(f"\n{'─'*40}")
        print(f"  {wname}: {wcfg['label']} (window={ws})")
        print(f"{'─'*40}")
        ...
        for seed in cfg['seeds']:
            ...
            for key in w48_sd:
                ...
                if key in m_sd and w48_sd[key].shape == m_sd[key].shape:
                    ...
                    transferred += 1
            model.load_state_dict(m_sd)
            print(f"  Transferred {transferred}/{len(w48_sd)} tensors")

            sp = os.path.join(cfg['output_dir'], f'exp475_{wname}_s{seed}.pth')
            train_bridge(model, train_x, val_x, sp, f'475-{wname}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                         future_steps=fs)
        ...
        for pinfo in data['per_patient']:
            ...
            for seed in cfg['seeds']:
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                m.load_state_dict(ckpt['model_state'])
                fp = os.path.join(cfg['output_dir'], f'exp475_{wname}_{pid}_s{seed}.pth')
                train_bridge(m, p_train, p_val, fp, f'475-{wname}-{pid}-s{seed}',
                             device, pk_mode=True,
                             epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                             lr=1e-4, future_steps=fs)
        ...
        result[wname] = {'per_patient': per_patient, 'average': avg,
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-475 Summary: Extended History Sweep")
    print(f"{'='*60}")
    for wname, wdata in result.items():
        ...
        keys = ['overall_mae', 'h30', 'h60', 'h90', 'h120', 'h150']
        vals = [f"{k}={avg.get(k,'—')}" for k in keys if k in avg]
        print(f"  {wname} (n={nt}): {', '.join(vals)}")

    # Delta vs w144 baseline
    base = result.get('w144_112_32', {}).get('average', {})
    for wname in ['w192_160_32', 'w240_208_32']:
        wavg = result.get(wname, {}).get('average', {})
        if base and wavg:
            print(f"\n  Delta ({wname} - w144):")
            for k in sorted(base.keys()):
                if k in wavg:
                    d = round(wavg[k] - base[k], 2)
                    print(f"    {k}: {d:+.2f}")

    _save_results(result, 'exp475_extended_history', cfg)
    return result


# ─── EXP-476: Longer Future Window Sweep ───

def run_exp476(args):
    """EXP-476: Does expanding the future window help long horizons?

    All our asymmetric experiments used 32 future steps (h160).
    What if we expand to 48 (h240) or 64 (h320) future steps
    while keeping ~9h history?

    More future steps = model sees further into PK absorption curves.
    But it also means less history at fixed window size.

    w144: 112h+32f (9h20m + 2h40m) vs 96h+48f (8h + 4h) vs 80h+64f (6h40m + 5h20m)
    w192: 160h+32f vs 144h+48f vs 128h+64f
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-476: Future Window Size Sweep")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp476_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'476-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']

    # Test on w192 with different future windows
    # w192 chosen because it gives enough room for both history and future
    ws = 192
    data = load_bridge_data(
        args.patients_dir, window_size=ws,
        max_patients=cfg['max_patients'], load_isf=True)
    ...
    future_configs = OrderedDict([
        ('f32_h160', {'future_steps': 32,
                      'label': '13h20m history + 2h40m future'}),
        ('f48_h144', {'future_steps': 48,
                      'label': '12h history + 4h future'}),
        ('f64_h128', {'future_steps': 64,
                      'label': '10h40m history + 5h20m future'}),
        ('f96_h96', {'future_steps': 96,
                     'label': '8h history + 8h future (symmetric)'}),
    ])

    result = {}
    for fname, fcfg in future_configs.items():
        fs = fcfg['future_steps']
        hs = ws - fs
        print(f"\n{'─'*40}")
        print(f"  {fname}: {fcfg['label']} ({hs}h + {fs}f)")
        print(f"{'─'*40}")
        ...
        for seed in cfg['seeds']:
            ...
            sp = os.path.join(cfg['output_dir'], f'exp476_{fname}_s{seed}.pth')
            train_bridge(model, train_x, val_x, sp, f'476-{fname}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                         future_steps=fs)
        ...
        for pinfo in data['per_patient']:
            ...
            for seed in cfg['seeds']:
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                m.load_state_dict(ckpt['model_state'])
                fp = os.path.join(cfg['output_dir'], f'exp476_{fname}_{pid}_s{seed}.pth')
                train_bridge(m, p_train, p_val, fp, f'476-{fname}-{pid}-s{seed}',
                             device, pk_mode=True,
                             epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                             lr=1e-4, future_steps=fs)
        ...
        result[fname] = {'per_patient': per_patient, 'average': avg}
        # Show horizons that exist for this future window
        print(f"\n  {fname} avg:")
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-476 Summary: Future Window Size Sweep (w192)")
    print(f"{'='*60}")
    for fname, fdata in result.items():
        avg = fdata['average']
        common = ['overall_mae', 'h30', 'h60', 'h90', 'h120', 'h150']
        vals = [f"{k}={avg.get(k,'—')}" for k in common if k in avg]
        print(f"  {fname}: {', '.join(vals)}")

    # Also show extended horizons
    print(f"\n  Extended horizons (where available):")
    for fname, fdata in result.items():
        avg = fdata['average']
        ext = [k for k in sorted(avg.keys()) if k.startswith('h') and
               int(k[1:]) > 160]
        if ext:
            vals = [f"{k}={avg[k]}" for k in ext]
            print(f"  {fname}: {', '.join(vals)}")

    _save_results(result, 'exp476_future_window_sweep', cfg)
    return result


# ─── EXP-477: Data Augmentation for w144 ───

def run_exp477(args):
    """EXP-477: Data augmentation to combat w144 data scarcity.

    w144 has only 3448 train windows vs w48's 10360 (3× less).
    EXP-475 showed w192/w240 fail partly due to even less data.

    Augmentations that preserve PK physics:
    1. Gaussian noise on glucose (sensor noise simulation)
    2. Time jitter (±1 step shift of entire sequence)
    3. Scale augmentation (±5% glucose amplitude, ISF-compatible)

    These should help w144 close the data gap without changing architecture.
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-477: Data Augmentation for w144 Asymmetric")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp477_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'477-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']
    ...
    def augment_data(train_x, noise_std=0.02, scale_range=0.05):
        """Create augmented copies preserving PK physics."""
        aug_list = [train_x]  # original

        # Gaussian noise on glucose channel only (channel 0)
        noisy = train_x.clone()
        noisy[:, :, 0] += torch.randn_like(noisy[:, :, 0]) * noise_std
        aug_list.append(noisy)

        # Scale augmentation: ±scale_range on glucose channel
        for s in [1.0 - scale_range, 1.0 + scale_range]:
            scaled = train_x.clone()
            scaled[:, :, 0] *= s
            aug_list.append(scaled)

        # Time shift: shift by +1 step (trim start, pad end with repeat)
        shifted = train_x.clone()
        shifted[:, :-1, :] = train_x[:, 1:, :]
        aug_list.append(shifted)

        return torch.cat(aug_list, dim=0)

    variants = OrderedDict([
        ('no_aug', {'augment': False, 'label': 'w144 asym, no augmentation'}),
        ('aug_4x', {'augment': True, 'label': 'w144 asym, 4× augmentation'}),
    ])
    ...
    for vname, vcfg in variants.items():
        ...
        t_data = augment_data(train144) if vcfg['augment'] else train144
        print(f"  Training windows: {t_data.shape[0]} ({'augmented' if vcfg['augment'] else 'original'})")
        ...
        for seed in cfg['seeds']:
            ...
            sp = os.path.join(cfg['output_dir'], f'exp477_{vname}_s{seed}.pth')
            train_bridge(model, t_data, val144, sp, f'477-{vname}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                         future_steps=future_steps)
        ...
        for pinfo in data144['per_patient']:
            ...
            ti, te = pinfo['train_idx']
            vi, ve = pinfo['val_idx']
            p_train = train144[ti:te]  # FT on original (not augmented)
            p_val = val144[vi:ve]
            p_isf = isf_v144[vi:ve] if isf_v144 is not None else None
            ...
            for seed in cfg['seeds']:
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                m.load_state_dict(ckpt['model_state'])
                fp = os.path.join(cfg['output_dir'], f'exp477_{vname}_{pid}_s{seed}.pth')
                train_bridge(m, p_train, p_val, fp, f'477-{vname}-{pid}-s{seed}',
                             device, pk_mode=True,
                             epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                             lr=1e-4, future_steps=future_steps)
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-477 Summary: Data Augmentation")
    print(f"{'='*60}")
    for vname, vdata in result.items():
        avg = vdata['average']
        keys = ['overall_mae', 'h30', 'h60', 'h90', 'h120', 'h150']
    ...
    no_aug = result.get('no_aug', {}).get('average', {})
    aug = result.get('aug_4x', {}).get('average', {})
    if no_aug and aug:
        print(f"\n  Delta (aug_4x - no_aug):")
        for k in sorted(no_aug.keys()):
            if k in aug:
                d = round(aug[k] - no_aug[k], 2)
                print(f"    {k}: {d:+.2f}")

    _save_results(result, 'exp477_data_augmentation', cfg)
    return result


# ─── EXP-478: Multi-Window Ensemble ───

def run_exp478(args):
    """EXP-478: Ensemble w48 and w144 predictions for best of both worlds.

    w48 has more training data (10360 vs 3448) and excels at h5-h30.
    w144 excels at h60-h150 with more context.

    Hypothesis: Simple averaging of w48 and w144 predictions at overlapping
    horizons gives better accuracy than either alone.

    Also test weighted ensemble: w48 dominates h5-h30, w144 dominates h60+,
    linear blend in between.
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-478: Multi-Window Ensemble (w48 + w144)")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")

    arch = {'d_model': 64, 'nhead': 4, 'num_layers': 4}

    # === Train w48 model (symmetric, 24h+24f) ===
    data48 = load_bridge_data(
        args.patients_dir, window_size=48,
        max_patients=cfg['max_patients'], load_isf=True)
    has_isf = 'isf_val' in data48
    ...
    print(f"\n  Train w48 ({n_ch}ch, {train48.shape[0]} windows)")
    w48_models = {}
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp478_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'478-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)

    # Per-patient FT for w48
    for pinfo in data48['per_patient']:
        pid = pinfo['name']
        ti, te = pinfo['train_idx']
        vi, ve = pinfo['val_idx']
        ...
        for seed in cfg['seeds']:
            ...
            ckpt = torch.load(sp, map_location=device, weights_only=False)
            m.load_state_dict(ckpt['model_state'])
            fp = os.path.join(cfg['output_dir'], f'exp478_w48_{pid}_s{seed}.pth')
            train_bridge(m, p_train, p_val, fp, f'478-w48-{pid}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                         lr=1e-4)
            w48_models[(pid, seed)] = fp

    # === Train w144 asym (112h+32f) with w48 transfer ===
    data144 = load_bridge_data(
        args.patients_dir, window_size=144,
        max_patients=cfg['max_patients'], load_isf=True)
    isf_v144 = data144.get('isf_val')
    ...
    print(f"\n  Train w144 asym ({train144.shape[0]} windows) with transfer")
    # Get w48 base states for transfer
    w48_base_states = {}
    for seed in cfg['seeds']:
        sp = os.path.join(cfg['output_dir'], f'exp478_w48_s{seed}.pth')
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_base_states[seed] = ckpt['model_state']

    w144_models = {}
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        model = PKGroupedEncoder(input_dim=n_ch, **arch)
        w48_sd = w48_base_states[seed]
        m_sd = model.state_dict()
        for key in w48_sd:
            if 'pos_encoder' in key: continue
            if key in m_sd and w48_sd[key].shape == m_sd[key].shape:
        ...
        sp = os.path.join(cfg['output_dir'], f'exp478_w144_s{seed}.pth')
        train_bridge(model, train144, val144, sp, f'478-w144-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                     future_steps=future_steps)

    # Per-patient FT for w144
    for pinfo in data144['per_patient']:
        pid = pinfo['name']
        ti, te = pinfo['train_idx']
        vi, ve = pinfo['val_idx']
        ...
        for seed in cfg['seeds']:
            ...
            ckpt = torch.load(sp, map_location=device, weights_only=False)
            m.load_state_dict(ckpt['model_state'])
            fp = os.path.join(cfg['output_dir'], f'exp478_w144_{pid}_s{seed}.pth')
            train_bridge(m, p_train, p_val, fp, f'478-w144-{pid}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                         lr=1e-4, future_steps=future_steps)
            w144_models[(pid, seed)] = fp

    # === Evaluate: individual + ensemble ===
    result = {'w48_only': {}, 'w144_only': {}, 'ensemble_avg': {}, 'ensemble_blend': {}}

    for pinfo in data144['per_patient']:
        pid = pinfo['name']
        vi48 = None
        # Find matching patient in data48
        for p48 in data48['per_patient']:
            if p48['name'] == pid:
                vi48 = p48['val_idx']
                break
        ...
        vi48_s, vi48_e = vi48
        p_val48 = val48[vi48_s:vi48_e]
        isf48 = data48.get('isf_val')
        p_isf48 = isf48[vi48_s:vi48_e] if isf48 is not None else None

        vi, ve = pinfo['val_idx']
        p_val144 = val144[vi:ve]
        p_isf144 = isf_v144[vi:ve] if isf_v144 is not None else None

        for seed in cfg['seeds']:
            # w48 predictions
            m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
            ckpt48 = torch.load(w48_models[(pid, seed)], map_location=device, weights_only=False)
            m48.load_state_dict(ckpt48['model_state'])
            r48 = evaluate_model(m48, p_val48, device, pk_mode=True, isf_val=p_isf48)
            result['w48_only'][pid] = r48

            # w144 predictions
            m144 = PKGroupedEncoder(input_dim=n_ch, **arch)
            ckpt144 = torch.load(w144_models[(pid, seed)], map_location=device, weights_only=False)
            m144.load_state_dict(ckpt144['model_state'])
            r144 = evaluate_model(m144, p_val144, device, pk_mode=True,
                                   isf_val=p_isf144, future_steps=future_steps)
            result['w144_only'][pid] = r144

            # Ensemble: collect raw predictions from both models
            m48.eval().to(device)
            m144.eval().to(device)

            isf_scale48 = p_isf48[0].item() if p_isf48 is not None else 1.0
            scale48 = isf_scale48 if isf_scale48 > 1 else 400.0
            isf_scale144 = p_isf144[0].item() if p_isf144 is not None else 1.0
            scale144 = isf_scale144 if isf_scale144 > 1 else 400.0

            # Since windows are different sizes and counts, we evaluate on
            # common horizons and compute MAE from per-step predictions
            # For simplicity, compare the MAE metrics directly
            ens_avg = {}
            ens_blend = {}
            common_keys = set(r48.keys()) & set(r144.keys())
            for k in common_keys:
                # Simple average of MAE — this is a conservative estimate
                # True ensemble would average predictions, not errors
                ens_avg[k] = round((r48[k] + r144[k]) / 2, 2)
                # Blend: w48 weight decreases with horizon
                if k.startswith('h'):
                    h = int(k[1:])
                    w48_weight = max(0.0, 1.0 - h / 120.0)  # 1.0 at h0, 0.0 at h120+
                    w144_weight = 1.0 - w48_weight
                    ens_blend[k] = round(r48[k] * w48_weight + r144[k] * w144_weight, 2)
                else:
                    ens_blend[k] = ens_avg[k]

            result['ensemble_avg'][pid] = ens_avg
            result['ensemble_blend'][pid] = ens_blend
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-478 Summary: Multi-Window Ensemble")
    print(f"{'='*60}")
    for mode in ['w48_only', 'w144_only', 'ensemble_avg', 'ensemble_blend']:
        avg = result.get(f'{mode}_avg', {})
        keys = ['overall_mae', 'h30', 'h60', 'h90', 'h120']
        vals = [f"{k}={avg.get(k,'—')}" for k in keys if k in avg]
        print(f"  {mode}: {', '.join(vals)}")

    _save_results(result, 'exp478_multi_window_ensemble', cfg)
    return result


# ─── EXP-479: Hard Patient Extended FT ───

def run_exp479(args):
    """EXP-479: Extended fine-tuning for hard patients.

    Patient a consistently has the highest val_loss (~0.65+) across experiments.
    Patient b (ISF=94) also struggles (val_loss ~0.19).

    Hypothesis: Hard patients need longer FT (30-50 epochs) and/or
    lower learning rate (5e-5) to converge properly.
    Also test larger batch fine-tuning (batch 8 → better gradient estimates
    on small per-patient datasets).
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-479: Hard Patient Extended Fine-Tuning")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp479_w48_s{seed}.pth')
        train_bridge(m48, train48, val48, sp, f'479-w48-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']

    # Train w144 base with transfer
    data144 = load_bridge_data(
        args.patients_dir, window_size=144,
        max_patients=cfg['max_patients'], load_isf=True)
    isf_v144 = data144.get('isf_val')
    ...
    for seed in cfg['seeds']:
        ...
        sp = os.path.join(cfg['output_dir'], f'exp479_w144_s{seed}.pth')
        train_bridge(model, train144, val144, sp, f'479-w144-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                     future_steps=future_steps)

    # FT variants
    ft_configs = OrderedDict([
        ('ft15_lr1e4', {'epochs': 15, 'lr': 1e-4, 'patience': 10,
                        'label': '15ep, lr=1e-4 (baseline)'}),
        ('ft30_lr1e4', {'epochs': 30, 'lr': 1e-4, 'patience': 15,
                        'label': '30ep, lr=1e-4'}),
        ('ft50_lr5e5', {'epochs': 50, 'lr': 5e-5, 'patience': 20,
                        'label': '50ep, lr=5e-5 (slow convergence)'}),
        ('ft50_lr1e4', {'epochs': 50, 'lr': 1e-4, 'patience': 20,
                        'label': '50ep, lr=1e-4 (longer training)'}),
    ])

    result = {}
    for ftname, ftcfg in ft_configs.items():
        print(f"\n{'─'*40}")
        print(f"  {ftname}: {ftcfg['label']}")
        print(f"{'─'*40}")
        ...
        for pinfo in data144['per_patient']:
            ...
            for seed in cfg['seeds']:
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                m.load_state_dict(ckpt['model_state'])
                fp = os.path.join(cfg['output_dir'], f'exp479_{ftname}_{pid}_s{seed}.pth')
                train_bridge(m, p_train, p_val, fp, f'479-{ftname}-{pid}-s{seed}',
                             device, pk_mode=True,
                             epochs=ftcfg['epochs'], patience=ftcfg['patience'],
                             lr_patience=5, lr=ftcfg['lr'],
                             future_steps=future_steps)
        ...
        result[ftname] = {'per_patient': per_patient, 'average': avg}
        keys = ['overall_mae', 'h30', 'h60', 'h90', 'h120']
        vals = [f"{k}={avg.get(k,'—')}" for k in keys if k in avg]
        print(f"\n  {ftname} avg: {', '.join(vals)}")
        # Show per-patient detail for hard patients
        for pid in ['a', 'b']:
            if pid in per_patient:
                r = per_patient[pid]
                print(f"    {pid}: overall={r.get('overall_mae','?')}, "
                      f"h60={r.get('h60','?')}")

    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-479 Summary: Extended Fine-Tuning")
    print(f"{'='*60}")
    for ftname, ftdata in result.items():
        avg = ftdata['average']
        keys = ['overall_mae', 'h30', 'h60', 'h90', 'h120']
        vals = [f"{k}={avg.get(k,'—')}" for k in keys if k in avg]
        print(f"  {ftname}: {', '.join(vals)}")
        # Hard patient breakdown
        for pid in ['a', 'b']:
            if pid in ftdata['per_patient']:
                r = ftdata['per_patient'][pid]
                print(f"    {pid}: mae={r.get('overall_mae','?')}, h60={r.get('h60','?')}")

    _save_results(result, 'exp479_hard_patient_ft', cfg)
    return result
...
# ─── EXP-609: Horizon Routing w48+w144 ───

def run_exp609(args):
    """EXP-609: Horizon routing — best model per horizon band.

    Current best results by window:
      - w48 d1+FT: h30=12.85, h60=17.08, h90=19.93, h120=22.13
      - w144 d1+transfer: h30=16.34, h120=24.34, h180=25.24, h360=29.62

    w48 dominates h30-h120 (more data, shorter prediction).
    w144 is needed for h120+ (longer context + future).

    Routing strategy: use w48 model for h30-h120, w144 model for h120+.
    This is a virtual ensemble — no joint training, just evaluation routing.

    Evaluates per-patient: train both w48 and w144 models, then report
    the best horizon from each for a "routed" composite score.
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-609: Horizon Routing w48 + w144")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")
    ...
    # ── Train w48 d1 model (specialist for h30-h120) ──
    data48 = load_bridge_data(
        args.patients_dir, window_size=48,
        max_patients=cfg['max_patients'], load_isf=True)
    has_isf = 'isf_val' in data48
    ...
    print(f"  w48: {n_ch_d1}ch, {train_d1_48.shape[0]} train windows")

    w48_states = {}
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch_d1, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp609_w48_d1_s{seed}.pth')
        train_bridge(m48, train_d1_48, val_d1_48, sp, f'609-w48-d1-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']

    # Per-patient w48 fine-tuning + evaluation
    w48_results = {}
    for pinfo in data48['per_patient']:
        pid = pinfo['name']
        ti, te = pinfo['train_idx']
        ...
        p_isf = data48.get('isf_val')
        p_isf = p_isf[vi:ve] if p_isf is not None else None

        for seed in cfg['seeds']:
            torch.manual_seed(seed); np.random.seed(seed)
            m = PKGroupedEncoder(input_dim=n_ch_d1, **arch)
            sp = os.path.join(cfg['output_dir'], f'exp609_w48_d1_s{seed}.pth')
            ckpt = torch.load(sp, map_location=device, weights_only=False)
            m.load_state_dict(ckpt['model_state'])
            fp = os.path.join(cfg['output_dir'], f'exp609_w48_{pid}_s{seed}.pth')
            train_bridge(m, p_train, p_val, fp, f'609-w48-{pid}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_ft'], patience=10, lr_patience=5, lr=1e-4)
            w48_results[pid] = evaluate_model(m, p_val, device, pk_mode=True, isf_val=p_isf)

    # ── Train w144 d1+transfer model (specialist for h120+) ──
    data144 = load_bridge_data(
        args.patients_dir, window_size=144,
        max_patients=cfg['max_patients'], load_isf=True)
    isf_v144 = data144.get('isf_val')
    ...
    print(f"\n  w144: {n_ch_d1}ch, {train_d1_144.shape[0]} train, "
          f"future_steps={future_steps_ext}")

    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m144 = PKGroupedEncoder(input_dim=n_ch_d1, **arch)
        # Transfer from w48
        src = w48_states[seed]
        tgt = m144.state_dict()
        transferred = 0
        for k in src:
            if k in tgt and 'pos_encoder' not in k and src[k].shape == tgt[k].shape:
                tgt[k] = src[k]
                transferred += 1
        m144.load_state_dict(tgt)
        print(f"  Transferred {transferred} params from w48")

        sp = os.path.join(cfg['output_dir'], f'exp609_w144_d1_s{seed}.pth')
        train_bridge(m144, train_d1_144, val_d1_144, sp, f'609-w144-d1-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                     future_steps=future_steps_ext)

    # Per-patient w144 fine-tuning + evaluation
    w144_results = {}
    for pinfo in data144['per_patient']:
        pid = pinfo['name']
        ti, te = pinfo['train_idx']
        ...
        for seed in cfg['seeds']:
            ...
            ckpt = torch.load(sp, map_location=device, weights_only=False)
            m.load_state_dict(ckpt['model_state'])
            fp = os.path.join(cfg['output_dir'], f'exp609_w144_{pid}_s{seed}.pth')
            train_bridge(m, p_train, p_val, fp, f'609-w144-{pid}-s{seed}',
                         device, pk_mode=True,
                         epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                         lr=1e-4, future_steps=future_steps_ext)
            w144_results[pid] = evaluate_model(m, p_val, device, pk_mode=True,
                                                isf_val=p_isf, future_steps=future_steps_ext)

    # ── Build routed composite ──
    # w48 for h30-h120, w144 for h150-h360
    short_horizons = ['h30', 'h60', 'h90', 'h120']
    long_horizons = ['h150', 'h180', 'h240', 'h300', 'h360']

    routed = {}
    for pid in w48_results:
        r = {}
        for h in short_horizons:
            if h in w48_results[pid]:
                r[h] = w48_results[pid][h]
        if pid in w144_results:
            for h in long_horizons:
                if h in w144_results[pid]:
                    r[h] = w144_results[pid][h]
        # Compute routed overall as average of all available horizons
        all_vals = [v for k, v in r.items() if k.startswith('h')]
        if all_vals:
            r['overall_mae'] = round(np.mean(all_vals), 2)
        routed[pid] = r

    # Average across patients
    def _avg_results(per_patient):
        avg = {}
        for pid, rep in per_patient.items():
            for k, v in rep.items():
                if k not in avg: avg[k] = []
                avg[k].append(v)
        return {k: round(np.mean(v), 2) for k, v in avg.items()}

    w48_avg = _avg_results(w48_results)
    w144_avg = _avg_results(w144_results)
    routed_avg = _avg_results(routed)

    result = {
        'w48_specialist': {'per_patient': w48_results, 'average': w48_avg},
        'w144_specialist': {'per_patient': w144_results, 'average': w144_avg},
        'routed': {'per_patient': routed, 'average': routed_avg},
    }

    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-609 Summary: Horizon Routing")
    print(f"{'='*60}")

    all_keys = ['overall_mae'] + short_horizons + long_horizons
    for label in ['w48_specialist', 'w144_specialist', 'routed']:
        avg = result[label]['average']
        vals = [f"{k}={avg.get(k,'—')}" for k in all_keys if k in avg]
        print(f"  {label}: {', '.join(vals)}")

    # Show routing benefit
    print(f"\n  Routing picks best of:")
    print(f"    h30-h120: w48 ({w48_avg.get('h120','—')} h120)")
    print(f"    h150-h360: w144 ({w144_avg.get('h360','—')} h360)")

    _save_results(result, 'exp609_horizon_routing', cfg)
    return result


# ─── EXP-610: w144 Stride Reduction ───

def run_exp610(args):
    """EXP-610: Increase w144 data volume via stride reduction.

    w144 has only 3,448 windows (stride=144). Reducing stride creates
    overlapping windows, increasing training data. Prior EXP-480 showed
    stride reduction helps at w48 (stride48 best).

    Tests:
      - stride144 (default): 3,448 windows
      - stride72 (50% overlap): ~6,900 windows
      - stride48 (67% overlap): ~10,350 windows

    Combined with d1+transfer, the best stride should approach w48's
    data volume advantage while keeping w144's extended horizon.
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-610: w144 Stride Reduction")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch_d1, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp610_w48_d1_s{seed}.pth')
        train_bridge(m48, train_d1_48, val_d1_48, sp, f'610-w48-d1-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']

    # ── Phase 2: w144 at different strides ──
    strides = [144, 72, 48]
    result = {}
    ...
    for stride in strides:
        ...
        for seed in cfg['seeds']:
            ...
            sp = os.path.join(cfg['output_dir'], f'exp610_{vname}_s{seed}.pth')
            train_bridge(model, train_d1_144, val_d1_144, sp,
                         f'610-{vname}-s{seed}', device, pk_mode=True,
                         epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                         future_steps=future_steps_ext)
        ...
        for pinfo in data144['per_patient']:
            ...
            for seed in cfg['seeds']:
                ...
                ftp = os.path.join(cfg['output_dir'], f'exp610_{vname}_{pid}_s{seed}.pth')
                train_bridge(ft_model, p_train, p_val, ftp,
                             f'610-{vname}-{pid}-s{seed}', device, pk_mode=True,
                             epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                             lr=1e-4, future_steps=future_steps_ext)
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-610 Summary: w144 Stride Reduction")
    print(f"{'='*60}")
    for vname, vdata in result.items():
        avg = vdata['average']
        nt = vdata['n_train']
    ...
    base_avg = result.get('stride144', {}).get('average', {})
    for vname in ['stride72', 'stride48']:
        vavg = result.get(vname, {}).get('average', {})
        if base_avg and vavg:
            print(f"\n  Delta ({vname} - stride144):")
            for k in sorted(vavg.keys()):
                if k in base_avg:
                    d = round(vavg[k] - base_avg[k], 2)
                    print(f"    {k}: {d:+.2f}")

    _save_results(result, 'exp610_stride_reduction', cfg)
    return result
...
# ════════════════════════════════════════════════════════════
# EXP-614: 3-Window Routing (w48 + w96 + w144)
# ════════════════════════════════════════════════════════════

def run_exp614(args):
    """EXP-614: Optimal 3-window routing system.

    Prior results show each window size has a sweet spot:
      w48:  best at h30-h120 (overall=16.38, 10360 windows)
      w96:  best at h120-h180 (h120=22.66, h150=24.23, 5176 windows)
      w144: only option for h180+ (h240=25.76, 3448 windows)

    This experiment trains all 3 specialists with d1+transfer and
    evaluates optimal routing boundaries vs each specialist alone.
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-614: 3-Window Routing")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")

    arch = {'d_model': 64, 'nhead': 4, 'num_layers': 4}

    # ── Phase 1: Train w48 base (shared for all transfers) ──
    data48 = load_bridge_data(
        args.patients_dir, window_size=48,
        max_patients=cfg['max_patients'], load_isf=True)
    has_isf = 'isf_val' in data48
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch_d1, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp614_w48_d1_s{seed}.pth')
        train_bridge(m48, train_d1_48, val_d1_48, sp, f'614-w48-d1-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']

    # ── w48 specialist: fine-tune and evaluate ──
    w48_pp = {}
    for pinfo in data48['per_patient']:
        pid = pinfo['name']
        ti, te = pinfo['train_idx']
        ...
        for seed in cfg['seeds']:
            torch.manual_seed(seed); np.random.seed(seed)
            ft = PKGroupedEncoder(input_dim=n_ch_d1, **arch)
            sp = os.path.join(cfg['output_dir'], f'exp614_w48_d1_s{seed}.pth')
            ckpt = torch.load(sp, map_location=device, weights_only=False)
            ft.load_state_dict(ckpt['model_state'])
            ftp = os.path.join(cfg['output_dir'], f'exp614_w48_{pid}_s{seed}.pth')
            train_bridge(ft, p_train, p_val, ftp,
                         f'614-w48-{pid}-s{seed}', device, pk_mode=True,
                         epochs=cfg['epochs_ft'], patience=10, lr_patience=5, lr=1e-4)
            w48_pp[pid] = evaluate_model(ft, p_val, device, pk_mode=True, isf_val=p_isf)
    ...
    # ── Helper: train extended model with transfer ──
    def train_extended(window_size, future_steps, prefix):
        history = window_size - future_steps
        data = load_bridge_data(
            args.patients_dir, window_size=window_size,
            max_patients=cfg['max_patients'], load_isf=True)
        isf_v = data.get('isf_val')
        train_d1, val_d1 = _prepare_pk_derivatives_asymmetric(
            data, history, use_isf=has_isf)
        n_train = train_d1.shape[0]
        print(f"\n  {prefix}: {n_train} train, hist={history} ({history*5}min), fut={future_steps}")
        ...
        for seed in cfg['seeds']:
            ...
            model.load_state_dict(tgt)
            if seed == cfg['seeds'][0]:
                print(f"    Transferred {xfr} params")
            sp = os.path.join(cfg['output_dir'], f'exp614_{prefix}_s{seed}.pth')
            train_bridge(model, train_d1, val_d1, sp,
                         f'614-{prefix}-s{seed}', device, pk_mode=True,
                         epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                         future_steps=future_steps)
        ...
        for pinfo in data['per_patient']:
            ...
            ti, te = pinfo['train_idx']
            vi, ve = pinfo['val_idx']
            p_train = train_d1[ti:te]; p_val = val_d1[vi:ve]
            p_isf = isf_v[vi:ve] if isf_v is not None else None
            for seed in cfg['seeds']:
                torch.manual_seed(seed); np.random.seed(seed)
                ft = PKGroupedEncoder(input_dim=n_ch_d1, **arch)
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                ft.load_state_dict(ckpt['model_state'])
                ftp = os.path.join(cfg['output_dir'], f'exp614_{prefix}_{pid}_s{seed}.pth')
                train_bridge(ft, p_train, p_val, ftp,
                             f'614-{prefix}-{pid}-s{seed}', device, pk_mode=True,
                             epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                             lr=1e-4, future_steps=future_steps)
                pp[pid] = evaluate_model(ft, p_val, device, pk_mode=True,
                                         isf_val=p_isf, future_steps=future_steps)
        avg = {}
        for pid, rep in pp.items():
            for k, v in rep.items():
                ...
                avg[k].append(v)
        avg = {k: round(np.mean(v), 2) for k, v in avg.items()}
        return {'per_patient': pp, 'average': avg, 'n_train': n_train}

    # ── Phase 2: w96 specialist (h160 config - best from EXP-612) ──
    w96_result = train_extended(96, 32, 'w96')

    # ── Phase 3: w144 specialist ──
    w144_result = train_extended(144, 72, 'w144')

    # ── Phase 4: Build routing table ──
    result = {
        'w48': {'per_patient': w48_pp, 'average': w48_avg, 'n_train': 10360},
        'w96': w96_result,
        'w144': w144_result,
    }

    # Routing: pick best model per horizon
    ...
    routed['overall_mae'] = round(np.mean(list(routed.values())), 2)
    result['routed'] = {'average': routed, 'routing_map': routing_map}

    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-614 Summary: 3-Window Routing")
    print(f"{'='*60}")
    for mname in ['w48', 'w96', 'w144']:
        avg = result[mname]['average']
        nt = result[mname]['n_train']
        keys = ['overall_mae', 'h30', 'h60', 'h90', 'h120', 'h150',
                'h180', 'h240', 'h300', 'h360']
        vals = [f"{k}={avg.get(k,'—')}" for k in keys if k in avg]
        print(f"  {mname} ({nt}win): {', '.join(vals)}")

    print(f"\n  ROUTED (best per horizon):")
    for h in all_horizons:
        if h in routed and h in routing_map:
            print(f"    {h}: {routed[h]} ← {routing_map[h]}")
    print(f"    overall: {routed['overall_mae']}")

    _save_results(result, 'exp614_3window_routing', cfg)
    return result


# ════════════════════════════════════════════════════════════
# EXP-615: w96 Stride + History Optimization
# ════════════════════════════════════════════════════════════

def run_exp615(args):
    """EXP-615: Optimize w96 configuration.

    EXP-612 showed w96 is a sweet spot. Now optimize:
      - stride24 on w96: ~10,000 windows (approaching w48's volume)
      - w96_h200: 56 history + 40 future (balanced)
      - w96_h160_stride24: more data + h160 reach

    Key question: can w96 with aggressive stride match w48's
    data volume while extending to h200?
    """
    cfg = _get_config(args)
    device = get_device(args.device)
    print(f"\n{'='*60}")
    print(f"EXP-615: w96 Stride + History Optimization")
    print(f"  seeds={cfg['seeds']}, base_ep={cfg['epochs_base']}")
    print(f"{'='*60}")
    ...
    for seed in cfg['seeds']:
        torch.manual_seed(seed); np.random.seed(seed)
        m48 = PKGroupedEncoder(input_dim=n_ch_d1, **arch)
        sp = os.path.join(cfg['output_dir'], f'exp615_w48_d1_s{seed}.pth')
        train_bridge(m48, train_d1_48, val_d1_48, sp, f'615-w48-d1-s{seed}',
                     device, pk_mode=True,
                     epochs=cfg['epochs_base'], patience=20, lr_patience=7)
        ckpt = torch.load(sp, map_location=device, weights_only=False)
        w48_states[seed] = ckpt['model_state']

    # ── Phase 2: w96 variants ──
    configs = {
        'w96_h160_s32': {'stride': 32, 'future_steps': 32},  # baseline from EXP-612
        'w96_h160_s24': {'stride': 24, 'future_steps': 32},  # more data
        'w96_h200_s32': {'stride': 32, 'future_steps': 40},  # longer horizon
        'w96_h200_s24': {'stride': 24, 'future_steps': 40},  # both
    }

    result = {}
    for vname, vcfg in configs.items():
        stride = vcfg['stride']
        fsteps = vcfg['future_steps']
        history = 96 - fsteps

        data96 = load_bridge_data(
            args.patients_dir, window_size=96, stride=stride,
            max_patients=cfg['max_patients'], load_isf=True)
        isf_v = data96.get('isf_val')
        train_d1, val_d1 = _prepare_pk_derivatives_asymmetric(
            data96, history, use_isf=has_isf)

        n_train = train_d1.shape[0]
        print(f"\n{'─'*40}")
        print(f"  {vname}: {n_train} train, hist={history} ({history*5}min), fut={fsteps} ({fsteps*5}min)")
        print(f"{'─'*40}")
        ...
        for seed in cfg['seeds']:
            ...
            model.load_state_dict(tgt)
            if seed == cfg['seeds'][0]:
                print(f"    Transferred {xfr} params, {n_train} windows")

            sp = os.path.join(cfg['output_dir'], f'exp615_{vname}_s{seed}.pth')
            train_bridge(model, train_d1, val_d1, sp,
                         f'615-{vname}-s{seed}', device, pk_mode=True,
                         epochs=cfg['epochs_base'], patience=20, lr_patience=7,
                         future_steps=fsteps)
        ...
        for pinfo in data96['per_patient']:
            ...
            for seed in cfg['seeds']:
                ...
                ckpt = torch.load(sp, map_location=device, weights_only=False)
                ft.load_state_dict(ckpt['model_state'])
                ftp = os.path.join(cfg['output_dir'], f'exp615_{vname}_{pid}_s{seed}.pth')
                train_bridge(ft, p_train, p_val, ftp,
                             f'615-{vname}-{pid}-s{seed}', device, pk_mode=True,
                             epochs=cfg['epochs_ft'], patience=10, lr_patience=5,
                             lr=1e-4, future_steps=fsteps)
                per_patient[pid] = evaluate_model(ft, p_val, device, pk_mode=True,
                                                  isf_val=p_isf, future_steps=fsteps)
        ...
        avg = {k: round(np.mean(v), 2) for k, v in avg.items()}
        result[vname] = {'per_patient': per_patient, 'average': avg,
                         'n_train': n_train, 'config': vcfg}

        keys = ['overall_mae', 'h30', 'h60', 'h90', 'h120', 'h150',
                'h180', 'h200', 'h240']
        vals = [f"{k}={avg.get(k,'—')}" for k in keys if k in avg]
        print(f"\n  {vname} ({n_train} win): {', '.join(vals)}")

    # Summary
    print(f"\n{'='*60}")
    print(f"EXP-615 Summary: w96 Optimization")
    print(f"{'='*60}")
    for vname, vdata in result.items():
        avg = vdata['average']
        nt = vdata['n_train']
    ...
    base = result.get('w96_h160_s32', {}).get('average', {})
    for vname in ['w96_h160_s24', 'w96_h200_s32', 'w96_h200_s24']:
        vavg = result.get(vname, {}).get('average', {})
        if base and vavg:
            print(f"\n  Delta ({vname} - baseline):")
            for k in sorted(vavg.keys()):
                if k in base:
                    d = round(vavg[k] - base[k], 2)
                    print(f"    {k}: {d:+.2f}")
