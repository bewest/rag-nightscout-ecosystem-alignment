...
EXP-2949 established that grid `iob` and `insulin_activity` are
controller-specific bookkeeping that may differ in conventions across
designs. This experiment re-derives BOTH iob and activity uniformly
from event history (bolus + bolus_smb + basal deviation) using a
standard biexponential insulin action curve (peak 75 min, DIA 300 min,
matching common humalog/novolog parameters).

If EXP-2944's iob_delta gap (Loop +0.59 climbing vs oref1 -0.04 peaking
during the 60-min sustained-high window) reproduces with the uniform
curve, the IOB-age mechanism is independent of grid `iob` column source.

Anchor: same as EXP-2944. BG crosses 180 climbing, prior 30min <180,
no carbs ±60min.
...
OUT = REPO / "externals" / "experiments" / "exp-2950_summary.json"
...
def main():
    ...
    for pid, sub in g.groupby("patient_id"):
        ...
        for i in range(LOOKBACK_CELLS, n - 12):
            ...
            rows.append({
                ...
                "bg_entry": float(bg[i]),
                "bg_end": float(bg[i_end]),
                "bg_delta": float(bg[i_end] - bg[i]),
            })
    ...
    summary = ev.groupby("design").agg(
        ...
        iob_end=("synth_iob_end", "mean"),
        act_end=("synth_act_end", "mean"),
        iob_delta=("synth_iob_delta", "mean"),
        act_delta=("synth_act_delta", "mean"),
        bg_delta=("bg_delta", "mean"),
    ).round(4)
    print("=== Per design (uniform biexponential action curve) ===")
    print(summary.to_string())

    print("\n=== Loop_AB_ON vs oref1 contrasts ===")
    for col in ["synth_iob_entry", "synth_act_entry", "synth_iob_delta",
                "synth_act_delta", "bg_delta"]:
