"""EXP-2969 - Per-patient SMB-channel velocity-coupling at PP.

EXP-2964 found pooled SMB-channel slopes near-identical between
Loop_AB_ON (+0.380) and oref1 (+0.361). Test whether this near-tie
holds per-patient (replicating EXP-2962 pattern). MWU between designs,
sign-test on positive direction within each design.
...
OUT = REPO / "externals" / "experiments" / "exp-2969_summary.json"
...
def main():
    ...
    print(f"Total PP events: {len(ev):,}")
    ...
    summary = []
    print("\n=== Per-design SMB-channel slope distribution ===")
    ...
    for (a_d, b_d) in [("Loop_AB_ON", "oref1"), ("oref1", "Loop_AB_OFF"),
        ...
        if len(a) >= 3 and len(b) >= 3:
            ...
            print(f"\n  MWU {a_d} vs {b_d} (two-sided): U={mw_two.statistic:.1f} p={mw_two.pvalue:.4g}")
            mwu_out[f"{a_d}_vs_{b_d}"] = {"U": float(mw_two.statistic),
                                          "p_two_sided": float(mw_two.pvalue),
                                          "a_n": len(a), "b_n": len(b)}

    out = {
        "scope": "Per-patient SMB-channel velocity-coupling at PP",
        "n_events": int(len(ev)),
