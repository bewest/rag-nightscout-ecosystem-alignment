...
class DetermineBasalSMB(private val outUnits: String = "mg/dL") {
    ...
    fun convert_bg(value: Double): String {
        return if (outUnits == "mmol/L") DecimalFormat("0.0").format(round(value * Constants.MGDL_TO_MMOLL, 1)).replace("-0.0", "0.0") else Math.round(value).toString()
    }
    ...
    fun determine_basal(
        ...
        iobArray.forEach { iobTick ->
            ...
            if (iobTick.iobWithZeroTemp!!.activity.isNaN() || sens.isNaN())
                System.err.println("iobTick.iobWithZeroTemp!!.activity=${iobTick.iobWithZeroTemp!!.activity} sens=$sens")
            val predZTBGI =
