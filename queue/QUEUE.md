...
- `[integration]` _(cwd: `externals/work/crm-bf-alarms`)_ `TEST=api.googlehome npm run test-single`
  - BF-31's sibling. Same environmental blocker as api.alexa - port 27034 has no mongod.
...
**Blast radius.** 2 commits. lib/server/cache.js, lib/server/dataloader.js, lib/api/entries.
...
**Notes.** Sequencing letter B. T0.2 passed its gate (0.837 -> 0.025 ms, asserted identical over HTTP). T0.3 did not. A dead `mills` write at dataloader.js:203 was found and removed with a test that goes red if it returns.
...
**Notes.** Sequencing letter F. Merging this in the connector repository ships it to NOBODY - cgm-remote-monitor pins by tarball. P0-TAG and P0-PIN are what deliver it.
