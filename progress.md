...
**Update (EXP-3457, later 2026-06-30)**: Added a combined basal+ISF safety stress. This explicitly separates "low ISF fits delta-RMSE" from "lower programmed ISF is safe": programming ISF 27.6-30 would add correction insulin, especially if paired with the +10% morning basal step. The coarse stress test does not show a large TBR increase, but it depends on speculative correction-opportunity assumptions and still lacks strict-UAM-clean correction evidence. The decision remains: do not combine basal and lower ISF; keep basal-first, single-parameter titration.
